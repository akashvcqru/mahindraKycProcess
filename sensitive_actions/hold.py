...
def hold_claim(claim, reason, webapp_url=None):
    """Mark a claim as HOLD and sync changes to local JSON, local Excel, and Google Sheets."""
    logging.info(f"[Manual Hold] Setting hold on claim for customer: {claim.get('customer_name')} (Row {claim.get('row_idx') + 1})")
    ...
    if os.path.exists(history_path):
        try:
            ...
            logging.info("[Manual Hold] Updated ui_history.json successfully.")
        except Exception as e:
            logging.error(f"[Manual Hold] Failed to update history JSON: {e}")
        ...
        try:
            ...
            for r in range(2, ws.max_row + 1):
                if ws.cell(row=r, column=1).value == row_num:
                    ws.cell(row=r, column=9, value="HOLD")
                    ws.cell(row=r, column=10, value=reason)
                    updated = True
                    break
            if updated:
                wb.save(excel_path)
                logging.info("[Manual Hold] Updated local Excel spreadsheet successfully.")
        except Exception as e:
            logging.error(f"[Manual Hold] Failed to update local Excel: {e}")
    ...
    if webapp_url:
        ...
        try:
            ...
            if resp.status_code == 200:
                logging.info("[Manual Hold] Google Sheet synchronized successfully.")
                return True
            else:
                logging.warning(f"[Manual Hold] Google Sheet sync returned HTTP {resp.status_code}")
        except Exception as e:
            logging.error(f"[Manual Hold] Google Sheet sync failed: {e}")
    return True
