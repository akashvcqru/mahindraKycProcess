...
def reject_claim(claim, reason, webapp_url=None):
    """Mark a claim as REJECTED and sync changes to local JSON, local Excel, and Google Sheets."""
    logging.info(f"[Manual Rejection] Rejecting claim for customer: {claim.get('customer_name')} (Row {claim.get('row_idx') + 1})")
        ...
        try:
            ...
            for item in history:
                if item.get("row_idx") == claim.get("row_idx") and item.get("customer_name") == claim.get("customer_name"):
                    ...
                    item["hold_reasons"] = reason
                    break
            with open(history_path, "w", encoding="utf-8") as f:
                json.dump(history, f, indent=2)
            logging.info("[Manual Rejection] Updated ui_history.json successfully.")
        except Exception as e:
            logging.error(f"[Manual Rejection] Failed to update history JSON: {e}")
        ...
        try:
            ...
            for r in range(2, ws.max_row + 1):
                if ws.cell(row=r, column=1).value == row_num:
                    ws.cell(row=r, column=9, value="REJECTED")
                    ws.cell(row=r, column=10, value=reason)
                    updated = True
                    break
            if updated:
                wb.save(excel_path)
                logging.info("[Manual Rejection] Updated local Excel spreadsheet successfully.")
        except Exception as e:
            logging.error(f"[Manual Rejection] Failed to update local Excel: {e}")
    ...
    if webapp_url:
        ...
        try:
            ...
            if resp.status_code == 200:
                logging.info("[Manual Rejection] Google Sheet synchronized successfully.")
                return True
            else:
                logging.warning(f"[Manual Rejection] Google Sheet sync returned HTTP {resp.status_code}")
        except Exception as e:
            logging.error(f"[Manual Rejection] Google Sheet sync failed: {e}")
    return True
