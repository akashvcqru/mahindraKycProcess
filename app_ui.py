...
class AppUI(tk.Tk):
    ...
    def setup_styles(self):
        ...
        style.configure(".", background=BG_COLOR, foreground=TEXT_COLOR, fieldbackground=CARD_BG_COLOR)
        ...
        style.configure("TNotebook.Tab", background=CARD_BG_COLOR, foreground=TEXT_LIGHT_COLOR, padding=[10, 5], font=("Segoe UI", 9))
        ...
        # Combobox
        style.configure("TCombobox", fieldbackground="#2A2A2A", background=BG_COLOR, foreground=TEXT_COLOR)
        
        # Scrollbar
        style.configure("Vertical.TScrollbar", gripcount=0, background="#2A2A2A", troughcolor=CARD_BG_COLOR, borderwidth=0, arrowsize=10)
    ...
    def create_layout(self):
        ...
        title_lbl = tk.Label(self.header_frame, text="MAHINDRA KYC PROCESS AUTOMATION", bg=CARD_BG_COLOR, fg="#60A5FA", font=("Segoe UI", 14, "bold"))
        title_lbl.pack(side="left", padx=20, pady=15)
        
        self.zone_lbl = tk.Label(self.header_frame, text="Active Zone: COMMON", bg=CARD_BG_COLOR, fg="#F59E0B", font=("Segoe UI", 11, "bold"))
        self.zone_lbl.pack(side="left", padx=(10, 20), pady=15)
        
        # Minimize to FAB action button
        self.fab_toggle_btn = tk.Button(self.header_frame, text="Minimize to FAB", bg="#2D3748", fg=TEXT_COLOR, activebackground="#4A5568", activeforeground=TEXT_COLOR, relief="flat", font=("Segoe UI", 9, "bold"), bd=0, padx=12, pady=6, command=self.minimize_to_fab)
        self.fab_toggle_btn.pack(side="right", padx=20, pady=12)

        # 2. Main split panes
        self.main_pane = tk.PanedWindow(self, orient="horizontal", bg=BG_COLOR, bd=0, sashwidth=6, sashrelief="flat")
        ...
        self.controls_card = tk.LabelFrame(self.left_panel, text="Automation Parameters", bg=CARD_BG_COLOR, fg="#60A5FA", font=("Segoe UI", 10, "bold"), padx=15, pady=15, relief="solid", bd=1)
        self.controls_card.pack(fill="x", side="top", pady=(0, 10))
        
        # Login Choice Option
        tk.Label(self.controls_card, text="Login Choice Mode:", bg=CARD_BG_COLOR, fg=TEXT_COLOR, font=("Segoe UI", 9)).grid(row=0, column=0, sticky="w", pady=6)
        self.login_mode_var = tk.StringVar(value="Already Login")
        self.login_mode_combo = ttk.Combobox(self.controls_card, textvariable=self.login_mode_var, values=["Already Login", "New Login"], state="readonly", width=18)
        self.login_mode_combo.grid(row=0, column=1, sticky="w", padx=10, pady=6)
        
        # Claim Selection Choice
        tk.Label(self.controls_card, text="Claim Type Category:", bg=CARD_BG_COLOR, fg=TEXT_COLOR, font=("Segoe UI", 9)).grid(row=1, column=0, sticky="w", pady=6)
        self.claim_choice_var = tk.StringVar(value="Loyalty Claims")
        self.claim_choice_combo = ttk.Combobox(self.controls_card, textvariable=self.claim_choice_var, values=["Loyalty Claims", "Exchange Claim"], state="readonly", width=18)
        self.claim_choice_combo.grid(row=1, column=1, sticky="w", padx=10, pady=6)
        
        # Rows Limit Bounds Option
        tk.Label(self.controls_card, text="Row Bounds Limit:", bg=CARD_BG_COLOR, fg=TEXT_COLOR, font=("Segoe UI", 9)).grid(row=2, column=0, sticky="w", pady=6)
        self.row_limit_var = tk.StringVar(value="a")
        self.row_limit_combo = ttk.Combobox(self.controls_card, textvariable=self.row_limit_var, values=["a", "1", "2", "3", "4", "5", "1,3,5", "2-4"], state="normal", width=18)
        self.row_limit_combo.grid(row=2, column=1, sticky="w", padx=10, pady=6)
        
        # Helper text for custom rows
        self.row_helper_lbl = tk.Label(self.controls_card, text="* Type custom rows e.g. 1,3,5 or 2-4", bg=CARD_BG_COLOR, fg=TEXT_LIGHT_COLOR, font=("Segoe UI", 8, "italic"))
        self.row_helper_lbl.grid(row=3, column=0, columnspan=2, sticky="w", pady=(0, 4))
        
        # Action Buttons
        self.start_btn = tk.Button(self.controls_card, text="RUN AUTOMATION PROCESS", bg=ACCENT_COLOR, fg="#FFFFFF", activebackground=ACCENT_HOVER, activeforeground="#FFFFFF", relief="flat", font=("Segoe UI", 10, "bold"), pady=8, bd=0, command=self.start_automation)
        self.start_btn.grid(row=4, column=0, columnspan=2, sticky="ew", pady=(10, 0))
        
        # Real-time styled terminal console logs log widget
        self.log_card = tk.LabelFrame(self.left_panel, text="Real-time Execution Console", bg=CARD_BG_COLOR, fg="#60A5FA", font=("Segoe UI", 10, "bold"), relief="solid", bd=1)
        self.log_card.pack(fill="both", expand=True)
        
        self.log_console = tk.Text(self.log_card, bg="#0D0D0D", fg=TEXT_COLOR, insertbackground=TEXT_COLOR, wrap="word", font=("Consolas", 9), bd=0)
        ...
        self.right_split = tk.PanedWindow(self.right_panel, orient="horizontal", bg=BG_COLOR, bd=0, sashwidth=6, sashrelief="flat")
        self.right_split.pack(fill="both", expand=True)
        
        # Row History Treeview Panel (Middle-ish area)
        self.history_frame = tk.LabelFrame(self.right_split, text="Processed Rows History", bg=CARD_BG_COLOR, fg="#60A5FA", font=("Segoe UI", 10, "bold"), relief="solid", bd=1, width=280)
        self.history_frame.pack_propagate(False)
        self.right_split.add(self.history_frame)
        
        self.history_tree = ttk.Treeview(self.history_frame, columns=("Row", "Customer", "Status"), show="headings", selectmode="browse")
        ...
        self.history_tree.tag_configure("approved", foreground=SUCCESS_COLOR, font=("Segoe UI", 9, "bold"))
        ...
        self.placeholder_lbl = tk.Label(self.viewer_frame, text="Select a row from history list\nto inspect verification details & PDF document comparison.", bg=BG_COLOR, fg=TEXT_LIGHT_COLOR, font=("Segoe UI", 12, "italic"))
        ...
        self.comparison_split = tk.PanedWindow(self.details_container, orient="horizontal", bg=BG_COLOR, bd=0, sashwidth=6, sashrelief="flat")
        ...
        self.dash_tree = ttk.Treeview(self.tab_dash, columns=("Field", "Value"), show="headings")
        ...
        self.doc_tree = ttk.Treeview(self.tab_doc, columns=("Field", "Value"), show="headings")
        ...
        self.checks_text = tk.Text(self.tab_checks, bg="#0F0F0F", fg=TEXT_COLOR, font=("Segoe UI", 9), wrap="word", padx=10, pady=10, bd=0)
        ...
        self.checks_text.tag_config("title", foreground="#60A5FA", font=("Segoe UI", 10, "bold"))
        ...
        tk.Label(self.pdf_toolbar, text="Document:", bg=CARD_BG_COLOR, fg=TEXT_COLOR, font=("Segoe UI", 9)).pack(side="left", padx=10)
        self.doc_selector_var = tk.StringVar()
        self.doc_selector = ttk.Combobox(self.pdf_toolbar, textvariable=self.doc_selector_var, state="readonly", width=30)
        self.doc_selector.pack(side="left", padx=5)
        self.doc_selector.bind("<<ComboboxSelected>>", self.on_doc_combo_select)
        
        # Prev / Next
        self.prev_btn = tk.Button(self.pdf_toolbar, text=" ◀ ", bg="#2D3748", fg=TEXT_COLOR, activebackground="#4A5568", relief="flat", font=("Segoe UI", 9), bd=0, command=self.prev_pdf_page)
        self.prev_btn.pack(side="left", padx=(15, 5))
        
        self.page_label = tk.Label(self.pdf_toolbar, text="Page 0 of 0", bg=CARD_BG_COLOR, fg=TEXT_COLOR, font=("Segoe UI", 9))
        self.page_label.pack(side="left", padx=5)
        
        self.next_btn = tk.Button(self.pdf_toolbar, text=" ▶ ", bg="#2D3748", fg=TEXT_COLOR, activebackground="#4A5568", relief="flat", font=("Segoe UI", 9), bd=0, command=self.next_pdf_page)
        self.next_btn.pack(side="left", padx=5)
        
        # Zoom Controls
        self.zoom_out_btn = tk.Button(self.pdf_toolbar, text=" Zoom - ", bg="#2D3748", fg=TEXT_COLOR, activebackground="#4A5568", relief="flat", font=("Segoe UI", 9), bd=0, command=self.zoom_out_pdf)
        self.zoom_out_btn.pack(side="right", padx=5, pady=8)
        
        self.zoom_lbl = tk.Label(self.pdf_toolbar, text="120%", bg=CARD_BG_COLOR, fg=TEXT_COLOR, font=("Segoe UI", 9))
        self.zoom_lbl.pack(side="right", padx=5)
        
        self.zoom_in_btn = tk.Button(self.pdf_toolbar, text=" Zoom + ", bg="#2D3748", fg=TEXT_COLOR, activebackground="#4A5568", relief="flat", font=("Segoe UI", 9), bd=0, command=self.zoom_in_pdf)
        ...
        self.pdf_canvas = tk.Canvas(self.canvas_container, bg="#262626", highlightthickness=0)
        self.pdf_canvas.pack(fill="both", expand=True, side="left")
        
        pdf_vscroll = ttk.Scrollbar(self.canvas_container, orient="vertical", command=self.pdf_canvas.yview)
        pdf_vscroll.pack(fill="y", side="right")
        pdf_hscroll = ttk.Scrollbar(self.pdf_frame, orient="horizontal", command=self.pdf_canvas.xview)
    ...
    def append_log_message(self, msg):
        ...
        tag = None
        msg_upper = msg.upper()
        if "MISMATCH" in msg_upper or "FAILED" in msg_upper or "ERROR" in msg_upper or "CRITICAL" in msg_upper:
            tag = "error"
        elif "MATCH" in msg_upper or "APPROVED" in msg_upper or "FINE" in msg_upper or "✔" in msg:
            tag = "success"
    ...
    def start_automation(self):
        ...
        self.start_btn.configure(state="disabled", text="RUNNING PROCESS...", bg="#4B5563")
        ...
        self.automation_thread = threading.Thread(target=self.run_automation_task, daemon=True)
    ...
    def reset_ui_controls(self):
        self.start_btn.configure(state="normal", text="RUN AUTOMATION PROCESS", bg=ACCENT_COLOR)
    ...
    def handle_input_request(self, event=None):
        ...
        lbl = tk.Label(dialog, text=self.input_prompt, bg=CARD_BG_COLOR, fg=TEXT_COLOR, font=("Segoe UI", 10, "bold"), wraplength=420, justify="center")
        ...
        if self.input_type == "password":
            entry = tk.Entry(dialog, textvariable=var_res, show="*", bg="#2A2A2A", fg=TEXT_COLOR, insertbackground=TEXT_COLOR, font=("Segoe UI", 12), relief="flat", bd=2)
            entry.pack(pady=10, padx=40, fill="x")
            entry.focus_set()
        elif self.input_type == "otp":
            entry = tk.Entry(dialog, textvariable=var_res, bg="#2A2A2A", fg=TEXT_COLOR, insertbackground=TEXT_COLOR, font=("Segoe UI", 16, "bold"), relief="flat", bd=2, justify="center")
            entry.pack(pady=10, padx=120, fill="x")
            entry.focus_set()
        elif self.input_type == "dropdown" and self.input_options:
            combo = ttk.Combobox(dialog, values=self.input_options, state="readonly", font=("Segoe UI", 10))
            ...
            for opt in self.input_options:
                btn = tk.Button(btn_frame, text=opt, width=12, bg=ACCENT_COLOR, fg="#FFFFFF", activebackground=ACCENT_HOVER, relief="flat", font=("Segoe UI", 9, "bold"), bd=0, command=lambda o=opt: [var_res.set(o), dialog.destroy()])
                btn.pack(side="left", padx=10)
        else:
            entry = tk.Entry(dialog, textvariable=var_res, bg="#2A2A2A", fg=TEXT_COLOR, insertbackground=TEXT_COLOR, font=("Segoe UI", 10), relief="flat", bd=2)
            entry.pack(pady=10, padx=40, fill="x")
            entry.focus_set()
        ...
        if self.input_type != "choice" or not self.input_options:
            submit_btn = tk.Button(dialog, text="Submit Action", bg=ACCENT_COLOR, fg="#FFFFFF", activebackground=ACCENT_HOVER, relief="flat", font=("Segoe UI", 10, "bold"), width=15, bd=0, command=on_submit)
    ...
    def handle_row_complete(self, row_result):
        ...
        node_id = self.history_tree.insert(
            "", 
            "end", 
            values=(f"Row {row_result['row_idx'] + 1}", row_result["customer_name"], row_result["status"]),
            tags=(status_tag,)
        )
    ...
    def load_claim_to_viewer(self, claim):
        ...
        if doc_names:
            ...
            self.checks_text.insert("end", "No document validations processed.\n", "issue")
            self.checks_text.configure(state="disabled")
    ...
    def load_selected_pdf_document(self, doc_dict):
        ...
        # Status Header
        self.checks_text.insert("end", f"Document Class: {doc_dict.get('file_type') or 'UNKNOWN'}\n", "title")
        self.checks_text.insert("end", f"File Name: {doc_dict.get('file_name')}\n\n", "title")
        ...
        if validations:
            ...
            for rule_name, rule_res in validations.items():
                ...
                if "MATCH" in res_upper or "VALID" in res_upper or "OK" in res_upper or "TRUE" in res_upper:
                    self.checks_text.insert("end", f"{res_str}\n", "match")
                elif "MISMATCH" in res_upper or "INVALID" in res_upper or "FAILED" in res_upper or "FALSE" in res_upper:
                    self.checks_text.insert("end", f"{res_str}\n", "mismatch")
                else:
                    self.checks_text.insert("end", f"{res_str}\n")
        else:
            self.checks_text.insert("end", "No specific validation checks rules performed on this document type.\n", "issue")
    ...
    def create_fab_window(self):
        ...
        canvas = tk.Canvas(self.fab_window, width=70, height=70, bg="#1E1E1E", highlightthickness=0)
        canvas.pack(fill="both", expand=True)
        
        # Draw a beautiful circle
        circle = canvas.create_oval(5, 5, 65, 65, fill="#2563EB", outline="#60A5FA", width=3, activefill="#3B82F6")
        text = canvas.create_text(35, 35, text="KYC", fill="#FFFFFF", font=("Segoe UI", 11, "bold"))
