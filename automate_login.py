...
def save_row_to_history(row_result):
    import json
    history_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ui_history.json")
    history = []
    ...
    history = [r for r in history if not (r.get("customer_name") == row_result["customer_name"] and r.get("row_idx") == row_result["row_idx"])]
...
def check_and_close_running_edge():
    ...
    try:
        ...
        if "msedge.exe" in res.stdout:
            ...
            choice = get_ui_input("Would you like to automatically close all running Microsoft Edge windows? (y/n): ", "choice", ["y", "n"]).strip().lower()
            if choice == 'y':
...
def select_antd_dropdown_option(page, trigger_element_or_selector, option_text=None, select_first=False, ask_user=False, field_name=""):
    ...
    if ask_user:
        ...
        if UI_INPUT_CALLBACK:
            selected_text = get_ui_input(f"Select option for {field_name}", "dropdown", option_texts)
...
def is_valid_customer_name(name_str):
    ...
    invalid_keywords = [
        "CUSTOMER NAME", "CHASSIS", "INVOICE", "SCHEME", "DATE", "MODEL", "AMOUNT",
        "RELATIONSHIP", "STATUS", "ACTION", "REMARK", "VIEW", "VEHICLE", "REGISTRATION",
        "LOYALTY", "EXCHANGE", "SCRAPPAGE", "WELCOME", "BONUS"
    ]
            ...
            if row.count() > 0 and row.first.is_visible():
                ...
                if tds.count() > 0:
                    ...
                    if match:
                        ...
                        logging.info(f"Extracted dashboard total amount from row 7 (Layout B): {clean_val}")
                ...
                next_row_sel = (
                    ...
                    if is_full else
                    "div.ant-drawer-body table > tbody > tr:nth-child(8)"
                )
                ...
                if next_row.count() > 0 and next_row.first.is_visible():
                    ...
                    if tds_next.count() > 0:
                        ...
                        if match:
                            clean_val = match.group(1).replace(',', '')
                            logging.info(f"Extracted dashboard total amount from row 8 (Layout A): {clean_val}")
                            return float(clean_val)
    except Exception as e:
        ...
        for sel in selectors:
            ...
            if loc.count() > 0:
                for idx in range(loc.count()):
                    ...
                    if txt:
                        match = re.search(r'Claim Date:\s*[\w/\.-]+', txt, re.IGNORECASE)
                        if match:
                            date_str = match.group(0).replace('Claim Date:', '').replace('Claim Date :', '').strip()
                            return date_str
                        date_match = re.search(r'(\d{1,2}[\s/\.-]+\w+[\s/\.-]+\d{2,4}|\d{1,2}[\s/\.-]+\d{1,2}[\s/\.-]+\d{2,4})', txt)
                        if date_match:
...
def extract_new_vehicle_chassis_and_invoice(page):
    ...
    logging.info(f"Portal New Vehicle - Chassis: '{chassis_no}', Invoice No: '{invoice_no}'")
...
def extract_valid_dl(text):
    ...
    for m in matches:
        ...
        if len(clean) >= 12 and len(clean) <= 17 and clean[:2].isalpha() and clean[2:].isdigit():
...
def validate_east_zone_old_vehicle(
    portal_chassis_no,
    portal_reg_no,
    pan_document_text=None,
    dl_document_text=None
):
    ...
    if pan_number:
        ...
        if (
            portal_chassis_norm == pan_number_norm
            or portal_reg_norm == pan_number_norm
        ):
            result["status"] = "PASS"
    ...
    if dl_number:
        ...
        if (
            portal_chassis_norm == dl_number_norm
            or portal_reg_norm == dl_number_norm
        ):
            result["status"] = "PASS"
                    ...
                    if val:
                        ...
                        for line in lines:
                            if line.lower() != "dealer name" and line != "-":
                                logging.info(f"Extracted Dealer Name from left drawer pane: '{line}' using selector '{sel}'")
                                return line
...
def compare_values_robust(doc_val, web_val, fuzzy_threshold=80):
    ...
    if len(clean_doc) > 0 and len(clean_web) > 0:
        short_val, long_val = (clean_doc, clean_web) if len(clean_doc) < len(clean_web) else (clean_web, clean_doc)
    ...
    try:
        words1 = [w.strip().lower() for w in re.sub(r'[^A-Za-z\s]', ' ', doc_val).split() if w.strip()]
        words2 = [w.strip().lower() for w in re.sub(r'[^A-Za-z\s]', ' ', web_val).split() if w.strip()]
...
def is_valid_model(model_str):
    ...
    # Clear generic terms
    if model_upper in ["SPORT UTILITY VEHICLES", "SUV", "UTILITY VEHICLES", "UTILITY VEHICLE", "PASSENGER VEHICLES", "PASSENGER VEHICLE", "COMMERCIAL VEHICLES", "COMMERCIAL VEHICLE"]:
        return False
    # Must contain at least one of the known Mahindra model keywords
    keywords = ["VEERO", "THAR", "BOLERO", "XUV", "SCORPIO", "SUPRO", "TREO", "ALFA", "ZOR", "MARAZZO", "CAMPER", "MAXX", "PICKUP", "PICK UP", "VIVIDUX"]
...
def compare_vehicle_models_robust(doc_model, web_model):
    ...
    def normalize_model_name(name):
        if any(x in name for x in ["XUV300", "XUV3OO", "XUV3O", "XUV3XO", "VIVIDUX", "VIVIDUX20AM1WQ", "VIVIDUX20A"]):
            return "XUV3XO"
        if "VEERO" in name:
            return "VEERO"
        if "BOLERO" in name:
    ...
    if doc_words & web_words:
        ...
        significant = [w for w in common_words if w not in ["AX5", "AX7", "LX", "MX", "MX1", "MX3", "MX5", "DS", "MT", "WQ", "SD", "V6", "1.6XXL"]]
        if significant:
            return True
...
"""
    ...
    def classify_and_extract(file_path, text, claim_customer_name, claim_details=None, old_vehicle_details=None):
        logging.info(f"[East Zone Model] Classifying document: {os.path.basename(file_path)}")
    ...
    def verify_documents(target_dir, customer_name, claim_details=None, old_vehicle_details=None, claim_choice=None, dashboard_dealer_name=None, dashboard_scheme_type=None):
        logging.info(f"[East Zone Model] Executing East Zone specific validation pipeline for customer '{customer_name}'...")
    ...
    if base64_images is None:
        ...
        try:
            ...
            for page_num in range(len(doc)):
                ...
                pix = page.get_pixmap(dpi=200)  # Use 200 DPI for fast uploads and good legibility
...
""".replace("{filename_hint}", filename_hint)
    ...
    try:
        logging.info(f"Sending vision extraction request to OpenAI for {os.path.basename(pdf_path)}...")
        response = requests.post("https://api.openai.com/v1/chat/completions", headers=headers, json=payload, timeout=60)
        response.raise_for_status()
        res_json = response.json()
        content = res_json["choices"][0]["message"]["content"]
        extracted_data = json.loads(content)
        logging.info(f"Successfully received vision response from OpenAI: {extracted_data.get('document_type')}")
        return extracted_data
    except Exception as e:
        logging.error(f"OpenAI Vision API extraction failed for {os.path.basename(pdf_path)}: {e}")
        return None
...
def get_upright_page_image(page, filename, reader):
    ...
    # Render page at 200 DPI for both OCR rotation test and final base64 conversion
    ...
    logging.info(f"  Selected rotation for {filename}: {best_angle}° (Score: {best_score})")
...
def extract_text_hybrid(pdf_path):
    ...
    if local_text and not is_corrupt:
        ...
        logging.info("--> Running local OCR (with rotation detection) to ensure complete, non-truncated text...")
        try:
    ...
    openai_res = extract_details_via_openai(pdf_path, base64_images=base64_images if base64_images else None)
    if openai_res:
    ...
    logging.warning("--> OpenAI Vision API failed. Falling back to local hybrid extraction...")
...
def extract_disclaimer_spatial(file_path, claim_customer_name, claim_details=None):
    ...
    for page in doc:
        ...
        fields_order = [
            ...
            "Vehicle Make", "Vehicle Model", "Invoice No", "Disclaimer Date", "Invoice Date", 
            "Welcome Bonus Amount"
        ]
...
def classify_and_extract(file_path, text, claim_customer_name, claim_details=None, old_vehicle_details=None):
    cur_zone = globals().get("CURRENT_ZONE", "").strip().upper()
    if cur_zone == "EAST":
        east_res = EastZoneModel.classify_and_extract(file_path, text, claim_customer_name, claim_details, old_vehicle_details)
    ...
    if openai_res:
        ...
        if "CUSTOMER DISCLAIMER" in text_upper or "DISCLAIMER FOR WELCOME" in text_upper or "DISCLAIMER FOR LOYALTY" in text_upper:
            doc_type = "DISCLAIMER"
        
        if doc_type == "UNKNOWN":
            if "PAN" in filename_upper:
                doc_type = "PAN"
            elif "ADHAR" in filename_upper or "AADHAAR" in filename_upper or "ADHR" in filename_upper:
                doc_type = "ADHAR"
            elif "DL" in filename_upper or "DRIVING" in filename_upper or "LICENCE" in filename_upper or "LICENSE" in filename_upper:
                doc_type = "DL"
            elif "COD" in filename_upper or "DGLV" in filename_upper or "OEM" in filename_upper or "VAHAN" in filename_upper:
                doc_type = "COD"
            elif "DIS" in filename_upper or "DISCLAIMER" in filename_upper or "DSC" in filename_upper:
                doc_type = "DISCLAIMER"
            elif "LEDGER" in filename_upper or filename_upper.startswith("LED") or "STMT" in filename_upper or "STATEMENT" in filename_upper:
                doc_type = "LEDGER"
                ...
                if "PERMANENT ACCOUNT NUMBER" in text_upper or "INCOME TAX DEPARTMENT" in text_upper:
                    ...
                    doc_type = "ADHAR"
                      ...
                      "OEM SCRAPPING" in text_upper or
                      "VAHAN SCREENSHOT" in text_upper or
                      "VAHAN" in text_upper):
                    doc_type = "COD"
        ...
        # Name matching
        extracted_name = openai_res.get("customer_name") or openai_res.get("company_name") # fallback for proprietor/GST docs
        name_score = 0.0
        if extracted_name:
            name_score = fuzz.token_sort_ratio(extracted_name.lower(), claim_customer_name.lower())
            status_name, score_name = compare_values_robust(extracted_name, claim_customer_name)
            if status_name.startswith("MATCH") and score_name > name_score:
                name_score = score_name
                
            if old_owner_name:
                rel_name_score = fuzz.token_sort_ratio(extracted_name.lower(), old_owner_name.lower())
                status_rel, score_rel = compare_values_robust(extracted_name, old_owner_name)
        ...
        if is_pan:
            ...
            adhar_no = openai_res.get("document_number") or openai_res.get("aadhaar_number")
            ...
            cert_no = openai_res.get("document_number") or openai_res.get("reference_number") or openai_res.get("invoice_number")
            ...
            if not cert_no or not str(cert_no).upper().startswith("COD"):
                possibles = [openai_res.get("document_number"), openai_res.get("reference_number"), openai_res.get("invoice_number"), openai_res.get("remarks")]
                found_cod = False
                for p in possibles:
                    if p and str(p).upper().startswith("COD"):
                        ...
                        break
                if not found_cod:
                    cert_match = re.search(r'\b(C[OQ0][D0][A-Z0-9OoQ_]+)\b', text, re.IGNORECASE)
                    if cert_match:
            ...
            if not reg_no:
                ...
                if ref_num and re.match(r'^[A-Z]{2}\d{1,2}[A-Z]{1,3}\d{1,4}$', str(ref_num).strip().upper().replace(" ", "").replace("-", "")):
                    reg_no = str(ref_num).strip().upper().replace(" ", "").replace("-", "")
                else:
                    reg_no_match = re.search(r'Registration\s*(?:No)?\.?\s*([A-Z]{2}\d{1,2}[A-Z0-9]{1,8})', text, re.IGNORECASE)
                    if reg_no_match:
                        reg_no = reg_no_match.group(1).upper().replace(" ", "").replace("-", "")
                    else:
                        reg_no_match_simple = re.search(r'Reg\w*\s*(?:No)?\.?\s*([A-Z0-9]+)', text, re.IGNORECASE)
            ...
            if not reg_no or reg_no == "NOT_FOUND":
                reg_no = None
                ref_num = openai_res.get("reference_number")
                if ref_num and ref_num != "NOT_FOUND" and re.match(r'^[A-Z]{2}\d{1,2}[A-Z]{1,3}\d{1,4}$', str(ref_num).strip().upper().replace(" ", "").replace("-", "")):
                    reg_no = str(ref_num).strip().upper().replace(" ", "").replace("-", "")
                else:
                    reg_no_match = re.search(r'Registration\s*(?:No)?\.?\s*([A-Z]{2}\d{1,2}[A-Z0-9]{1,8})', text, re.IGNORECASE)
                    if reg_no_match:
            ...
            if not vehicle_model:
                model_match = re.search(r'Vehicle\s*Model\s*(?:\([^)]*\))?\s*[:\.-]?\s*([A-Za-z0-9\s]+?)(?:\s+OR|\n|$)', text, re.IGNORECASE)
                if model_match:
                    vehicle_model = model_match.group(1).strip()
            if not vehicle_model:
                for keyword in ["VEERO", "THAR", "BOLERO", "XUV", "SCORPIO", "SUPRO", "TREO", "ALFA", "ZOR", "VIVIDUX"]:
                    match = re.search(rf'\b({keyword}\s+[A-Z0-9\.\s-]+)\b', text, re.IGNORECASE)
                    if match:
                        vehicle_model = match.group(1).strip()
                        break
                    else:
                        match_simple = re.search(rf'\b({keyword})\b', text, re.IGNORECASE)
                        if match_simple:
            ...
            chassis_no = openai_res.get("chassis_number") or openai_res.get("document_number")
            ...
            welcome_bonus = openai_res.get("welcome_bonus_amount") or openai_res.get("amount")
            ...
            res_dict["extracted_data"] = {
                ...
                "Chassis No": None if (chassis_no and is_pan_format(str(chassis_no))) else (str(chassis_no).upper().strip() if chassis_no else None),
            ...
            if v_model == "NOT_FOUND": v_model = None
            if nv_model == "NOT_FOUND": nv_model = None
            if v_model:
                vm_upper = v_model.strip().upper()
                if vm_upper in ["SPORT UTILITY VEHICLES", "SUV", "UTILITY VEHICLES", "UTILITY VEHICLE", "PASSENGER VEHICLES", "PASSENGER VEHICLE", "COMMERCIAL VEHICLES", "COMMERCIAL VEHICLE"]:
                    v_model = None
            if nv_model:
                nvm_upper = nv_model.strip().upper()
                if nvm_upper in ["SPORT UTILITY VEHICLES", "SUV", "UTILITY VEHICLES", "UTILITY VEHICLE", "PASSENGER VEHICLES", "PASSENGER VEHICLE", "COMMERCIAL VEHICLES", "COMMERCIAL VEHICLE"]:
                    nv_model = None
            ...
            if not vehicle_model:
                ...
                if isinstance(p_details, list):
                    for item in p_details:
                        ...
                        for keyword in ["VEERO", "THAR", "BOLERO", "XUV", "SCORPIO", "SUPRO", "TREO", "ALFA", "ZOR"]:
                            if keyword in item_str:
                                if isinstance(item, dict):
                                    desc = item.get("description") or item.get("name") or item.get("model") or list(item.values())[0]
            ...
            if not chassis_no:
                ...
                if isinstance(p_details, list):
                    for item in p_details:
                        ...
                        vin_match = re.search(r'(?:VIN|CHS|CHASSIS)[:\.-]?([A-Z0-9]{17})', item_str)
                        if vin_match:
            ...
            if not chassis_no:
                vin_match = re.search(r'(?:VIN|CHS|CHASSIS)[:\.-]?\s*([A-Z0-9]{17})', text_upper.replace(" ", "").replace("-", ""))
                if vin_match:
                    chassis_no = vin_match.group(1)
                else:
                    match_m = re.search(r'\b(M[A-Z0-9]{16})\b', text_upper.replace(" ", "").replace("-", ""))
                    if match_m:
                        chassis_no = match_m.group(1)
    ...
    is_cod = ("COD" in filename or "DGLV" in filename or "OEM" in filename or "VAHAN" in filename) and not ("DISCLAIMER" in filename or "DIS" in filename or "DSC" in filename)
    is_disclaimer = "DIS" in filename or "DISCLAIMER" in filename or "DSC" in filename or "CD-" in filename
    is_adhar = "ADHAR" in filename or "AADHAAR" in filename or "ADHR" in filename
    is_ledger = "LEDGER" in filename or filename.startswith("LED") or "STMT" in filename or "STATEMENT" in filename
    ...
    if is_pan:
        ...
        try:
            ...
            if disclaimer_date == "NOT_FOUND": disclaimer_date = None
            if welcome_bonus == "NOT_FOUND": welcome_bonus = None
            
            if extracted_name:
                name_score = fuzz.token_sort_ratio(extracted_name.lower(), claim_customer_name.lower())
        except Exception as ocr_err:
            logging.error(f"Spatial OCR extraction failed: {ocr_err}. Falling back to flat regex.")
        ...
        if not disclaimer_date:
            all_date_matches = re.finditer(r'\bDate\s*[:\.-]?\s*(\d{2}[-/\.]\d{2}[-/\.]\d{4})\b', text, re.IGNORECASE)
        ...
        if not invoice_date:
            inv_date_match = re.search(r'Invoice\s*Date\s*[:\.-]?\s*(\d{2}[-/\.]\d{2}[-/\.]\d{4})', text, re.IGNORECASE)
            if inv_date_match:
...
def validate_ledger_conditions(text, filename, claim_details, current_zone, current_city, claim_choice, issues, dashboard_scheme_type=None):
    """
    ...
    if zone == "EAST":
        ...
        if model_group:
            try:
                ...
                scheme_expected_amount = get_contribution_for_scheme(model_group, contributions, ledger_scheme_type, city_name=city)
            except Exception as e:
        ...
        if is_loyalty:
            ...
            for line in lines:
                ...
                if "WELCOME" in line_norm and "BONUS" in line_norm:
                    ...
                    if scheme_expected_amount is not None and scheme_expected_amount not in targets:
            ...
            if not welcome_found:
                issues.append(f"Ledger [{filename}]: 'Welcome Bonus' not found in ledger (Required for East Zone)")
            elif not welcome_amt_match:
                primary_target = targets[0] if targets else "Not Found"
                issues.append(f"Ledger [{filename}]: Welcome Bonus amount mismatch in ledger. Expected: {primary_target}")
        else:
            ...
            if not scrappage_found:
                issues.append(f"Ledger [{filename}]: 'Scrappage Bonus' not found in ledger (Required for East Zone)")
            elif not scrappage_amt_match:
                primary_target = targets[0] if targets else "Not Found"
                issues.append(f"Ledger [{filename}]: Scrappage Bonus amount mismatch in ledger. Expected: {primary_target}")
    ...
    if zone in ["SOUTH", "NORTH", "WEST"]:
        ...
        if found_entry and not amt_match:
            ...
            issues.append(f"Ledger [{filename}]: Combined {zone} Zone entry amount mismatch in ledger. Dashboard expected: {primary_target} (Matched keyword: '{matched_keyword}')")
    ...
    if zone != "EAST":
        ...
        if scrappage_found and not scrappage_amt_match:
            primary_target = dashboard_targets[0] if dashboard_targets else "Not Found"
            issues.append(f"Ledger [{filename}]: Scrappage Bonus amount mismatch in ledger. Dashboard expected: {primary_target}")
...
def verify_ledger_stamp_and_signature(pdf_path, company_name):
    ...
    if openai_res and "seal_stamp_dealer_name" in openai_res:
        ...
        if openai_stamp_dealer:
            logging.info(f"OpenAI extracted seal stamp dealer name from ledger: '{openai_stamp_dealer}'")
            if company_variants:
                for variant in company_variants:
                    status_stamp, score_stamp = compare_values_robust(openai_stamp_dealer, variant)
                    if status_stamp.startswith("MATCH") or score_stamp >= 70:
                        return True, f"GOOD (Stamp/Signature verified via OpenAI: '{openai_stamp_dealer}' matches company '{variant}')"
                logging.warning(f"OpenAI stamp name '{openai_stamp_dealer}' did not match any variant of '{company_name}'")
            else:
                return True, f"GOOD (Stamp/Signature present via OpenAI: '{openai_stamp_dealer}', but no expected company name to compare against)"
    ...
    if openai_res and company_variants:
        ...
        for variant in company_variants:
            ...
            variant_words = [w for w in re.sub(r'[^A-Z]', ' ', variant_upper).split() if len(w) >= 4]
            matched_words = [w for w in variant_words if w in full_text]
            if len(variant_words) >= 2 and len(matched_words) >= max(1, len(variant_words) // 2):
                logging.info(f"Company name '{variant}' found in ledger document header/full_text (matched words: {matched_words})")
                return True, f"GOOD (Company name '{variant}' confirmed in ledger document header: matched words {matched_words})"
...
"""
        ...
        "messages": [
            {
                ...
                "content": [
                    {"type": "text", "text": prompt},
                    {
                        "type": "image_url",
                        "image_url": {
                            "url": f"data:image/png;base64,{inv_b64}"
                        }
                    },
                    {
                        "type": "image_url",
                        "image_url": {
                            "url": f"data:image/png;base64,{disc_b64}"
                        }
                    }
                ]
            }
    ...
    try:
        logging.info("Calling OpenAI Vision API for custom Invoice-Declaration cross-validation...")
        response = requests.post("https://api.openai.com/v1/chat/completions", headers=headers, json=payload, timeout=60)
        response.raise_for_status()
        res_json = response.json()
        content = res_json["choices"][0]["message"]["content"]
        ...
        printed_name_match_score = 0
        if p_inv and p_dec:
            printed_name_match_score = int(round(fuzz.token_sort_ratio(str(p_inv).lower(), str(p_dec).lower())))
        printed_name_match = (printed_name_match_score >= 80)
        
        # 2. Handwritten Name Match score (threshold 70)
        handwritten_name_match_score = 0
        if h_inv and h_dec:
            handwritten_name_match_score = int(round(fuzz.token_sort_ratio(str(h_inv).lower(), str(h_dec).lower())))
        handwritten_name_match = (handwritten_name_match_score >= 70)
        
        # 3. Printed vs Handwritten Match score (threshold 70) on same document
        inv_same_score = 0
        if p_inv and h_inv:
            inv_same_score = int(round(fuzz.token_sort_ratio(str(p_inv).lower(), str(h_inv).lower())))
            
        dec_same_score = 0
        if p_dec and h_dec:
            dec_same_score = int(round(fuzz.token_sort_ratio(str(p_dec).lower(), str(h_dec).lower())))
            
        printed_vs_handwritten_match_score = min(inv_same_score, dec_same_score) if (p_inv or h_inv) and (p_dec or h_dec) else 0
        ...
        if not printed_name_match:
            ...
            remarks.append(f"Printed customer name mismatch across documents: score {printed_name_match_score}")
...
def verify_invoice_stamp_and_signatures(pdf_path, company_name, customer_name):
    ...
    try:
        ...
        for box, txt, conf in ocr_results:
            ...
            if not dealer_anchor_150 and ("dealer seal" in txt_clean or "seal" in txt_clean or "authorized" in txt_clean or "authorized person" in txt_clean or "authorised" in txt_clean or "authorised signatory" in txt_clean or "dealer authorized" in txt_clean):
                xs = [pt[0] for pt in box]
                ys = [pt[1] for pt in box]
        ...
        logging.info(f"  Customer Signature region combined (blue+black) pixels: {sig_dark_pixels}")
        ...
        if company_variants:
            for variant in company_variants:
                variant_cleaned = re.sub(r'^(?:neither|nor|or|the|and|to|from|by|harmless)\s+', '', variant, flags=re.IGNORECASE)
                words = [wd.upper() for wd in re.sub(r'[^A-Za-z]', ' ', variant_cleaned).split() if len(wd) >= 3]
        ...
        if not stamp_ok:
            ...
            if not stamp_ok and company_words:
                ...
                if openai_res:
                    ...
                    for variant in company_variants:
                        variant_upper = variant.upper()
                        variant_key_words = [w for w in re.sub(r'[^A-Z]', ' ', variant_upper).split() if len(w) >= 5]
                        matched_hdr = [w for w in variant_key_words if w in full_text]
                        if len(variant_key_words) >= 2 and len(matched_hdr) >= max(1, len(variant_key_words) // 2):
                            stamp_ok = True
                            stamp_msg = f"GOOD (Company '{variant}' confirmed in document letterhead/header: matched words {matched_hdr})"
                            logging.info(f"  Stamp verified via document header for '{variant}': {matched_hdr}")
                            break
...
def validate_disclaimer_doc(filename, pdf_path, text, data, validations, claim_details, old_vehicle_details, claim_choice, dashboard_scheme_type, company_name, customer_name, issues):
    GREEN_TEXT = "\033[92m"
    RED_TEXT = "\033[91m"
    YELLOW_TEXT = "\033[93m"
    RESET_TEXT = "\033[0m"
    ...
    def extract_dates_from_disclaimer_text(text):
        ...
        # 1. Disclaimer Date: Date: DD/MM/YYYY, Date: DD-MM-YYYY, Date : DD-MM-YYYY, etc.
        # Find matches starting with Date or DATE
        date_matches = re.finditer(r'(?:Date|DATE)\s*[:\.-]?\s*(\d{1,2}[-/\.]\d{1,2}[-/\.]\d{2,4}|\d{1,2}[-/\.][A-Za-z]{3}[-/\.]\d{2,4})', text)
        ...
        inv_date_match = re.search(r'Invoice\s*Date\s*[:\.-_]?\s*(\d{1,2}[-/\.][A-Za-z]{3}[-/\.]\d{2,4}|\d{1,2}[-/\.]\d{1,2}[-/\.]\d{2,4})', text, re.IGNORECASE)
        if inv_date_match:
    ...
    if scheme_type == "welcome":
        ...
        is_new_format_present = (matching_new_kws >= 6) and ("CUSTOMER DISCLAIMER" in text_norm or "CONFIRM" in text_norm)
        
        if (has_old_format and not is_new_format_present) or matching_new_kws < 5:
            print(f"  - Disclaimer Format Check: {RED_TEXT}FAIL (Disclaimer format does not match required template. Found {matching_new_kws}/8 keywords){RESET_TEXT}")
            ...
            print(f"  - Disclaimer Format Check: {GREEN_TEXT}PASS (Disclaimer format matches template){RESET_TEXT}")
    else:
        print(f"  - Disclaimer Format Check: {GREEN_TEXT}PASS (Disclaimer format check skipped - Scrappage Scheme claim){RESET_TEXT}")
    ...
    if doc_chassis and is_pan_format(doc_chassis):
        logging.warning(f"Disclaimer [{filename}]: Extracted Chassis No '{doc_chassis}' matches PAN card format - likely misread PAN, not chassis")
        issues.append(f"Disclaimer [{filename}]: Chassis No '{doc_chassis}' appears to be a PAN card number (format: {doc_chassis}), not a valid chassis number")
        doc_chassis = None
    ...
    if is_veero:
        ...
        print(f"  - Disclaimer Extracted Welcome Bonus   : {doc_welcome_bonus or 'Not Found'}")
    ...
    if validations.get("Name Match Status") == "MATCH":
        print(f"  - Disclaimer Name Match Status    : {GREEN_TEXT}MATCH ({score:.1f}% Similarity){RESET_TEXT}")
    else:
        print(f"  - Disclaimer Name Match Status    : {RED_TEXT}MISMATCH ({score:.1f}% Similarity){RESET_TEXT}")
    ...
    if is_veero or scheme_type == "welcome":
        print(f"  - Disclaimer Old Vehicle Comparisons : {GREEN_TEXT}SKIPPED (Not required for {'VEERO' if is_veero else 'Welcome Bonus'} scheme){RESET_TEXT}")
        ...
        print(f"  - Disclaimer Old Vehicle Comparisons : {YELLOW_TEXT}SKIPPED (No website details available){RESET_TEXT}")
    ...
    if claim_details:
        ...
        if web_new_model and text:
            text_upper = text.upper()
            model_words = [w.upper() for w in re.sub(r'[^A-Za-z0-9]', ' ', web_new_model).split() if len(w) >= 3]
        ...
        color_new_model = GREEN_TEXT if model_word_found or not web_new_model else RED_TEXT
        ...
        if web_new_model:
            if model_word_found:
                print(f"    * Model  : '{web_new_model}' found in disclaimer (matched words: {model_matched_words}) -> {color_new_model}PASS{RESET_TEXT}")
            else:
                print(f"    * Model  : '{web_new_model}' NOT found in disclaimer text -> {color_new_model}MISMATCH{RESET_TEXT}")
        else:
        ...
        if not model_word_found and web_new_model:
            issues.append(f"Disclaimer [{filename}]: New Vehicle Model mismatch (portal model '{web_new_model}' not found in disclaimer)")
        ...
        web_inv_date = get_val_by_fuzzy_key(claim_details, ["Invoice Date", "InvoiceDate"])
        ...
        if doc_inv_date and doc_inv_date != "NOT_FOUND" and web_inv_date:
            ...
            if parsed_doc and parsed_web:
                if parsed_doc != parsed_web:
                    issues.append(f"Disclaimer [{filename}]: Invoice Date mismatch (doc: {doc_inv_date} vs web: {web_inv_date})")
            else:
                status_inv_date, _ = compare_values_robust(doc_inv_date, web_inv_date, fuzzy_threshold=100)
                if not status_inv_date.startswith("MATCH"):
                    issues.append(f"Disclaimer [{filename}]: Invoice Date mismatch (doc: {doc_inv_date} vs web: {web_inv_date})")

        # Extract Disclaimer Date and Invoice Date from disclaimer text
        doc_date_extracted, doc_inv_date_extracted = extract_dates_from_disclaimer_text(text)
        if not doc_date_extracted or doc_date_extracted == "NOT_FOUND":
            doc_date_extracted = doc_date if doc_date and doc_date != "NOT_FOUND" else None
        if not doc_inv_date_extracted or doc_inv_date_extracted == "NOT_FOUND":
            doc_inv_date_extracted = doc_inv_date if doc_inv_date and doc_inv_date != "NOT_FOUND" else None
            
        web_claim_date = claim_details.get("dashboard_claim_date") if claim_details else None
        ...
        if date_validation in ["FAIL", "HOLD"]:
            issues.append(f"Disclaimer [{filename}]: Disclaimer Date Validation {date_validation} - {date_validation_reason}")
            
        expected_amount = None
        if web_new_model:
            contributions = load_contribution_data()
            city_name = resolve_city_name(claim_details)
            expected_amount = find_matching_contribution(web_new_model, contributions, city_name=city_name)
        ...
        print(f"  - Disclaimer New Vehicle Comparisons : {YELLOW_TEXT}SKIPPED (No website claim details available){RESET_TEXT}")

def validate_invoice_doc(filename, pdf_path, text, data, validations, claim_details, company_name, customer_name, issues, dashboard_scheme_type=None, claim_choice=None):
    GREEN_TEXT = "\033[92m"
    RED_TEXT = "\033[91m"
    YELLOW_TEXT = "\033[93m"
    RESET_TEXT = "\033[0m"
    ...
    contributions = load_contribution_data()
    if vehicle_model:
        expected_amount = get_contribution_for_scheme(vehicle_model, contributions, scheme_type, city_name=city_name)
    if expected_amount is None and claim_details:
        web_model = get_val_by_fuzzy_key(claim_details, ["New vehicle Model Group", "Model Group", "New Vehicle Model"])
        if web_model:
            expected_amount = get_contribution_for_scheme(web_model, contributions, scheme_type, city_name=city_name)
            
    # Fallback to general lookup if scheme-specific is None
    if expected_amount is None:
        if vehicle_model:
            expected_amount = find_matching_contribution(vehicle_model, contributions, city_name=city_name)
        if expected_amount is None and claim_details:
            web_model = get_val_by_fuzzy_key(claim_details, ["New vehicle Model Group", "Model Group", "New Vehicle Model"])
            if web_model:
                expected_amount = find_matching_contribution(web_model, contributions, city_name=city_name)
    ...
    if expected_amount is not None:
        ...
        extracted_bonus = data.get("Welcome Bonus Amount") or data.get("welcome_bonus_amount") or data.get("Scrappage Bonus Amount") or data.get("scrappage_bonus_amount")
        ...
        normalized_text = (text_upper.replace("BGNUS", "BONUS")
        ...
        for pat in note_patterns:
            ...
            for match in matches:
                ...
                char_map = {
                    'O': '0', 'o': '0', 'U': '0', 'u': '0', 'I': '1', 'i': '1',
                    'L': '1', 'l': '1', 'S': '5', 's': '5', 'B': '8', 'b': '8',
                    'Z': '2', 'z': '2', 'G': '6', 'g': '6'
                }
            ...
            try:
                ...
                if matched_target is not None:
                    if matched_target == expected_amount:
                        print(f"  - Amount Match Status  : {GREEN_TEXT}MATCH (Extracted Bonus {extracted_bonus_val} matches Expected {expected_amount}){RESET_TEXT}")
                    else:
                        gst_pct = round((matched_target / expected_amount - 1.0) * 100)
                        print(f"  - Amount Match Status  : {GREEN_TEXT}MATCH (Extracted Bonus {extracted_bonus_val} matches GST-inclusive Expected {matched_target:.2f} ({gst_pct}% GST)){RESET_TEXT}")
                else:
                    print(f"  - Amount Match Status  : {RED_TEXT}MISMATCH (Extracted Bonus {extracted_bonus_val} vs Expected {expected_amount}){RESET_TEXT}")
                    issues.append(f"Invoice [{filename}]: Amount mismatch. Extracted Bonus: {extracted_bonus_val}, Expected: {expected_amount} (or GST-inclusive) for model '{vehicle_model}'")
            except ValueError:
                print(f"  - Amount Match Status  : {RED_TEXT}INVALID FORMAT ({extracted_bonus}){RESET_TEXT}")
                issues.append(f"Invoice [{filename}]: Amount has invalid format: {extracted_bonus}")
...
def common_verify_documents(target_dir, customer_name, claim_details=None, old_vehicle_details=None, claim_choice=None, dashboard_dealer_name=None, dashboard_scheme_type=None):
    ...
    if not company_name:
        for pdf_path in pdf_files:
            ...
            is_disclaimer = "DIS" in filename or "DISCLAIMER" in filename or "DSC" in filename
    ...
    for pdf_path in pdf_files:
        ...
        try:
            ...
            is_pan_file = (file_type == "PAN") or ("PAN" in filename.upper()) or ("PERMANENT ACCOUNT NUMBER" in text.upper()) or ("INCOME TAX DEPARTMENT" in text.upper())
            is_dl_file = (file_type == "DL") or any(kw in filename.upper() for kw in ["DL", "DRIVING", "LICENCE", "LICENSE"]) or ("DRIVING LICENCE" in text.upper()) or ("DRIVING LICENSE" in text.upper())
            ...
            if file_type == "PAN":
                ...
                if not is_relationship_self:
                    ...
                    for p_name in portal_names:
                        if is_name_in_text(text, p_name):
                            ...
                            break
                    if name_found:
                        print(f"  - Customer Name Check: {GREEN_TEXT}PASS (Portal customer name found in relative's PAN){RESET_TEXT}")
                    else:
                        print(f"  - Customer Name Check: {RED_TEXT}FAIL (Portal customer name '{customer_name}' not found in relative's PAN){RESET_TEXT}")
                        issues.append(f"PAN [{filename}]: Portal customer name '{customer_name}' not found in relative's PAN document to verify relationship")
                    
                if pan_no:
                    print(f"  - PAN Status       : {GREEN_TEXT}VERIFIED{RESET_TEXT}")
                    # Cross-check against Portal PAN detected in Chassis/Reg fields
                    portal_pan = claim_details.get("portal_detected_pan") if claim_details else None
                    if portal_pan and portal_pan != pan_no:
                        src_field = claim_details.get("portal_pan_dl_source_field", "Old Vehicle")
                        print(f"  - Portal PAN Check : {RED_TEXT}MISMATCH (Portal {src_field} has '{portal_pan}', but document has '{pan_no}'){RESET_TEXT}")
                        issues.append(f"PAN [{filename}]: PAN mismatch. Portal {src_field} shows '{portal_pan}', but PAN document shows '{pan_no}'")
                    elif portal_pan:
                        print(f"  - Portal PAN Check : {GREEN_TEXT}MATCH (Matches Portal {claim_details.get('portal_pan_dl_source_field')}){RESET_TEXT}")
                else:
                ...
                is_vahan_screenshot = "VAHAN" in filename.upper() or "OEM" in filename.upper()
                doc_issues = []
                ...
                if is_vahan_screenshot:
                    ...
                    if web_old_chassis:
                        if check_chassis_or_cert_in_text(text, web_old_chassis):
                            print(f"  - Certificate of Deposit Validation: {GREEN_TEXT}PASS (Expected COD '{web_old_chassis}' found in Vahan screenshot){RESET_TEXT}")
                            cert_match = True
                        else:
                            print(f"  - Certificate of Deposit Validation: {RED_TEXT}FAIL (Expected COD '{web_old_chassis}' not found in Vahan screenshot){RESET_TEXT}")
                            doc_issues.append(f"COD [{filename}]: Expected Certificate of Deposit '{web_old_chassis}' not found in Vahan screenshot.")
                    else:
                        print(f"  - Certificate of Deposit Validation: {YELLOW_TEXT}SKIPPED (No expected COD/old vehicle chassis in dashboard details){RESET_TEXT}")
                        cert_match = True
                        
                    # 2. Match New Vehicle Chassis Number from dashboard/claim details
                    web_new_chassis = None
                    if claim_details:
                        web_new_chassis = get_val_by_fuzzy_key(claim_details, ["Chassis No", "Chassis Number"])
                        
                    new_chassis_match = False
                    if web_new_chassis:
                        if check_chassis_or_cert_in_text(text, web_new_chassis):
                            print(f"  - New Vehicle Chassis Validation: {GREEN_TEXT}PASS (Expected New Chassis '{web_new_chassis}' found in Vahan screenshot){RESET_TEXT}")
                            new_chassis_match = True
                        else:
                            print(f"  - New Vehicle Chassis Validation: {RED_TEXT}FAIL (Expected New Chassis '{web_new_chassis}' not found in Vahan screenshot){RESET_TEXT}")
                            doc_issues.append(f"COD [{filename}]: Expected New Vehicle Chassis '{web_new_chassis}' not found in Vahan screenshot.")
                    else:
                        print(f"  - New Vehicle Chassis Validation: {YELLOW_TEXT}SKIPPED (No new vehicle chassis in dashboard details){RESET_TEXT}")
                        new_chassis_match = True
                    ...
                    if not is_relationship_self:
                        ...
                        for p_name in portal_names:
                            if is_name_in_text(text, p_name):
                                ...
                                break
                        if name_found:
                            print(f"  - Customer Name Check: {GREEN_TEXT}PASS (Portal customer name found in COD){RESET_TEXT}")
                        else:
                            print(f"  - Customer Name Check: {RED_TEXT}FAIL (Portal customer name '{customer_name}' not found in COD){RESET_TEXT}")
                            issues.append(f"COD [{filename}]: Portal customer name '{customer_name}' not found in COD document to verify relationship")
                ...
                if not is_relationship_self:
                    ...
                    for p_name in portal_names:
                        if is_name_in_text(text, p_name):
                            ...
                            break
                    if name_found:
                        print(f"  - Customer Name Check: {GREEN_TEXT}PASS (Portal customer name found in relative's Aadhaar){RESET_TEXT}")
                    else:
                        print(f"  - Customer Name Check: {RED_TEXT}FAIL (Portal customer name '{customer_name}' not found in relative's Aadhaar){RESET_TEXT}")
                        issues.append(f"Aadhaar [{filename}]: Portal customer name '{customer_name}' not found in relative's Aadhaar document to verify relationship")
                ...
                if not is_relationship_self:
                    ...
                    for p_name in portal_names:
                        if is_name_in_text(text, p_name):
                            ...
                            break
                    if name_found:
                        print(f"  - Customer Name Check: {GREEN_TEXT}PASS (Portal customer name found in relative's DL){RESET_TEXT}")
                    else:
                        print(f"  - Customer Name Check: {RED_TEXT}FAIL (Portal customer name '{customer_name}' not found in relative's DL){RESET_TEXT}")
                        issues.append(f"DL [{filename}]: Portal customer name '{customer_name}' not found in relative's DL document to verify relationship")
                        
                if dl_no:
                    print(f"  - DL Status        : {GREEN_TEXT}VERIFIED{RESET_TEXT}")
                    # Cross-check against Portal DL detected in Chassis/Reg fields
                    portal_dl = claim_details.get("portal_detected_dl") if claim_details else None
                    # Clean DL for comparison
                    clean_doc_dl = re.sub(r'[\s-]', '', dl_no) if dl_no else None
                    clean_portal_dl = re.sub(r'[\s-]', '', portal_dl) if portal_dl else None
                    
                    if clean_portal_dl and clean_doc_dl and clean_portal_dl != clean_doc_dl:
                        src_field = claim_details.get("portal_pan_dl_source_field", "Old Vehicle")
                        print(f"  - Portal DL Check  : {RED_TEXT}MISMATCH (Portal {src_field} has '{portal_dl}', but document has '{dl_no}'){RESET_TEXT}")
                        issues.append(f"DL [{filename}]: DL mismatch. Portal {src_field} shows '{portal_dl}', but DL document shows '{dl_no}'")
                    elif clean_portal_dl:
                        print(f"  - Portal DL Check  : {GREEN_TEXT}MATCH (Matches Portal {claim_details.get('portal_pan_dl_source_field')}){RESET_TEXT}")
                else:
                ...
                validate_disclaimer_doc(filename, pdf_path, text, data, validations, claim_details, old_vehicle_details, claim_choice, dashboard_scheme_type, company_name, customer_name, issues)
                # Check if it also contains invoice
                if "INV" in filename.upper() or "INVOICE" in filename.upper() or "TAX INVOICE" in text.upper() or "GST INVOICE" in text.upper():
                    print("  --- Running Merged Invoice Validation ---")
                    validate_invoice_doc(filename, pdf_path, text, data, validations, claim_details, company_name, customer_name, issues, dashboard_scheme_type=dashboard_scheme_type, claim_choice=claim_choice)
                ...
                if not scheme_val and claim_details:
                    scheme_val = get_val_by_fuzzy_key(claim_details, ["Scheme", "Scheme Type"])
                ...
                if not scheme_val:
                    ...
                    if not scheme_to_use:
                        is_loyalty = (claim_choice == "1" or claim_choice == 1 or str(claim_choice).lower() == "loyalty")
                        scheme_to_use = "welcome" if is_loyalty else "scrappage"
                    scheme_val = "Welcome Bonus" if scheme_to_use == "welcome" else "Scrappage Bonus"
                ...
                print(f"  - Selected Claim       : {expected_keyword_desc} (from Scheme field '{scheme_val}')")
                    ...
                    if dash_tot is not None:
                        ...
                        for k, v in claim_details.items():
                            if k.strip().lower() == "total amount":
                                try:
                                    expected_amount = float(''.join(c for c in v if c.isdigit() or c == '.'))
                ...
                if claim_details:
                    model_group = get_val_by_fuzzy_key(claim_details, ["New vehicle Model Group", "Model Group", "New Vehicle Model"])
                if expected_amount is None and model_group and ledger_scheme_type is not None:
                    contributions = load_contribution_data()
                    city_name = resolve_city_name(claim_details)
                    expected_amount = get_contribution_for_scheme(model_group, contributions, ledger_scheme_type, city_name=city_name)
                    from_source = f"Google Sheet for {model_group}"
                    
                if expected_amount is not None:
                    print(f"  - Expected Amount      : {expected_amount} (from {from_source})")
                else:
                    print(f"  - Expected Amount      : {YELLOW_TEXT}UNKNOWN (Dashboard Total Amount missing and Vehicle '{model_group}' not found in schemes){RESET_TEXT}")
                ...
                if ledger_scheme_type is None:
                    issues.append(f"Ledger [{filename}]: Scheme mismatch. Vehicle details scheme is '{scheme_val}', expected Welcome Bonus or Scrappage Bonus")
                elif expected_amount is None:
                    # Mismatch of scheme data - brand not found in Google sheet
                    issues.append(f"Ledger [{filename}]: Expected amount is unknown (Dashboard Total Amount missing and Vehicle '{model_group}' not found in {expected_keyword_desc} schemes)")
                else:
                    lines = text.split("\n")
                    for line in lines:
                    ...
                    if found_scheme_entry:
                        ...
                        print(f"    * Match Status       : {GREEN_TEXT}GOOD (Scheme '{expected_keyword_desc}', Amount {matched_amount}, and Narration matched!){RESET_TEXT}")
                    else:
                        print(f"  - Ledger Entry         : {RED_TEXT}NOT FOUND or MISMATCHED{RESET_TEXT}")
                        if not found_keyword_at_all:
                            print(f"    * Match Status       : {RED_TEXT}FAIL (Could not find entry matching narration {expected_keyword_desc}){RESET_TEXT}")
                            issues.append(f"Ledger [{filename}]: '{expected_keyword_desc}' entry not found in ledger")
                        else:
                            print(f"    * Match Status       : {RED_TEXT}FAIL (Amount mismatch for '{expected_keyword_desc}' entry. Expected: {expected_amount}){RESET_TEXT}")
                            issues.append(f"Ledger [{filename}]: Amount mismatch for '{expected_keyword_desc}' entry. Expected: {expected_amount}")
                            
                # Call specific zone/city checks layered on top
                validate_ledger_conditions(text, filename, claim_details, CURRENT_ZONE, CURRENT_CITY, claim_choice, issues, dashboard_scheme_type=dashboard_scheme_type)
                ...
                if "DIS" in filename.upper() or "DSC" in filename.upper() or "DISCLAIMER" in filename.upper() or "CUSTOMER DISCLAIMER" in text.upper():
    ...
    if invoice_path and disclaimer_path:
        ...
        print("="*50)
        cross_res = validate_invoice_declaration_relationship(invoice_path, disclaimer_path)
        import json
    ...
    if current_zone_upper == "EAST":
        ...
        portal_reg_no = ""
        if old_vehicle_details:
            portal_chassis_no = get_val_by_fuzzy_key(old_vehicle_details, ["Chassis No", "Chassis Number"]) or ""
            portal_reg_no = get_val_by_fuzzy_key(old_vehicle_details, ["Reg No", "Registration No", "Registration Number"]) or ""
        ...
        ez_res = validate_east_zone_old_vehicle(
            portal_chassis_no,
            portal_reg_no
        )
        ...
        if ez_res.get("east_zone_validation") in ["FAIL", "HOLD"]:
            issues.append(f"East Zone Old Vehicle Validation: {ez_res.get('validation_reason', 'No reason')}")
...
def create_excel_results(excel_records, excel_path):
    ...
    try:
        ...
        headers = ["Row Number", "Customer Name", "Status", "Hold Reasons / Remarks", "Processed Date"]

        header_font = Font(name="Segoe UI", size=11, bold=True, color="FFFFFF")
        header_fill = PatternFill(start_color="1E3A8A", end_color="1E3A8A", fill_type="solid")  # Dark Blue
        align_center = Alignment(horizontal="center", vertical="center")
        align_left = Alignment(horizontal="left", vertical="center")
        status_approved_font = Font(name="Segoe UI", size=10, bold=True, color="047857")  # Green
        ...
        if os.path.exists(excel_path):
            try:
                ...
                logging.warning(f"Could not load existing Excel file ({load_err}). A new file will be created.")
                wb = None
        else:
            wb = None
        ...
        wb.save(excel_path)
        logging.info(f"Successfully saved {len(excel_records)} new record(s) to Excel sheet at: {excel_path}")
        return True
...
def main(use_existing_login=None, target_claim_choice=None, row_limit=None):
    ...
    try:
        ...
        logging.info(f"Launching Edge with profile path: {user_data_path}, profile directory: {profile_dir}")
    ...
    try:
        ...
        if is_already_logged_in or "/dashboard" in current_url:
            ...
            if not is_redirected_to_dashboard:
                if use_existing:
                    ...
                    email = get_ui_input("Enter your User ID (e.g. 50016105@mahindra.com): ", "text").strip()
                    if not email:
                    ...
                    def enter_otp_flow():
                        ...
                        otp = get_ui_input("Please enter the OTP sent to your text/number: ", "otp").strip()
        ...
        if target_claim_choice in ["1", "2"]:
            ...
            claim_choice = get_ui_input("Select option (1/2): ", "choice", ["1", "2"]).strip()
        ...
        if claim_choice == "1":
            ...
            custom_confirm = get_ui_input("Press Enter to use these dates, or type 'c' to enter custom dates: ", "choice", ["", "c"]).strip().lower()
            if custom_confirm == 'c':
                user_from = get_ui_input(f"Enter Claim From Date (DD/MM/YYYY) [{calculated_from}]: ", "text").strip()
                user_to = get_ui_input(f"Enter Claim To Date (DD/MM/YYYY) [{calculated_to}]: ", "text").strip()
                if user_from:
        ...
        try:
            ...
            if clicked_100:
                ...
                logging.warning("Could not find the '100' page-size option. Proceeding with current page size.")
        except Exception as ps_err:
            logging.warning(f"Could not set page size to 100: {ps_err}. Proceeding with current page size.")
        ...
        if row_limit is not None:
            ...
            print(f"{'='*55}")
            while True:
                row_choice = get_ui_input(f"  Enter your choice (a / number / range): ", "text").strip().lower()
                if row_choice == 'a':
                    rows_to_process = list(range(row_count))
                    break
                    ...
                    if parsed:
                        ...
                        print("  No valid rows matched. Please try again (e.g. 1-20, 1,3,5).")
                else:
                    print("  Invalid choice. Enter 'a', a number (e.g. 20), or a range (e.g. 1-20).")
        
        logging.info(f"Will process {len(rows_to_process)} row(s): {[r + 1 for r in rows_to_process]}")
        ...
        for loop_idx, row_idx in enumerate(rows_to_process):
            ...
            print(f"\n{'='*60}")
            print(f"  PROCESSING ROW {row_idx + 1} ({loop_idx + 1} of {len(rows_to_process)})")
            print(f"{'='*60}")
            ...
            portal_new_chassis, portal_new_invoice = extract_new_vehicle_chassis_and_invoice(page)
            if portal_new_chassis:
                claim_details["Chassis No"] = portal_new_chassis
                logging.info(f"Portal New Vehicle Chassis No (direct selector): '{portal_new_chassis}'")
            if portal_new_invoice:
                claim_details["Invoice No"] = portal_new_invoice
                logging.info(f"Portal New Vehicle Invoice No (direct selector): '{portal_new_invoice}'")
            ...
            try:
                ...
                sel_model_full = f"{base_tbody} > tr:nth-child(12) > td:nth-child(3) > div > span"
                ...
                for sel in [sel_model_full, sel_model_generic]:
                    ...
                    if loc.count() > 0 and loc.first.is_visible():
                        ...
                        if model_group_val and model_group_val != "-":
                            ...
                            logging.info(f"Portal New Vehicle Model Group (direct selector): '{model_group_val}'")
                            break
            except Exception as e:
                logging.warning(f"Error extracting New Vehicle Model Group from portal: {e}")

            # Scheme check at tr:nth-child(7) > th:nth-child(1)
            scheme_text = ""
            try:
                sel1 = "body > div:nth-child(11) > div > div.ant-drawer-content-wrapper > div > div.ant-drawer-body > div > div.ant-col.ant-col-xs-24.ant-col-sm-24.ant-col-md-18.ant-col-lg-18.ant-col-xl-18.ant-col-xxl-18.css-1442l13 > div > form > div.ant-row.app_drawerBodyRight__LGAX0.css-1442l13 > div > div.ant-card.ant-card-bordered.css-1442l13 > div > div > div > table > tbody > tr:nth-child(7) > th:nth-child(1) > div > span"
                sel2 = "div.ant-drawer-body table > tbody > tr:nth-child(7) > th:nth-child(1) > div > span"
                ...
                logging.warning(f"Error checking scheme selector: {e}")
            
            if not scheme_text and claim_details:
                # Check keys first
                for k in claim_details.keys():
                    if "welcome" in k.lower() or "scrappage" in k.lower() or "bonus" in k.lower():
                        scheme_text = k.strip()
                        break
                # Check values if still not found
                if not scheme_text:
                    for k, v in claim_details.items():
                        v_lower = str(v).lower()
                        if "welcome" in v_lower or "bonus" in v_lower or "scrappage" in v_lower:
                            scheme_text = str(v).strip()
                            break
            
            # Final fallback: try extract_scheme_type_from_old_vehicle_details
            if not scheme_text or scheme_text.lower() in ["total amount", ""]:
                try:
                    fallback_scheme = extract_scheme_type_from_old_vehicle_details(page)
                    if fallback_scheme:
                        scheme_text = fallback_scheme
                        logging.info(f"Row {row_idx + 1}: Scheme resolved from old vehicle details tab: '{scheme_text}'")
                except Exception as fb_err:
                    logging.warning(f"Fallback scheme extraction failed: {fb_err}")

            logging.info(f"Row {row_idx + 1}: Extracted Scheme Name: '{scheme_text}'")
            normalized_scheme = " ".join(scheme_text.lower().split())
            # If user selected Loyalty Claims, all rows are Welcome Bonus by default
            if claim_choice == "1":
                is_welcome_scheme = True
                if not any(kw in normalized_scheme for kw in ["welcome", "bonus"]):
                    logging.info(f"Row {row_idx + 1}: Loyalty Claims selected - treating as Welcome Bonus (drawer showed '{scheme_text}')")
            else:
                is_welcome_scheme = any(kw in normalized_scheme for kw in ["welcome", "bonus"])
            if not is_welcome_scheme:
                logging.info(f"Row {row_idx + 1}: Scheme is '{scheme_text}', not a Welcome Bonus scheme. Skipping this row.")
                try:
                    close_drawer_robust(page)
            ...
            if extra_names:
                logging.info(f"Extracted customer names from drawer selectors: {extra_names}")
            ...
            if dash_total_amt is not None:
                logging.info(f"Extracted Dashboard Total Amount from row 7: {dash_total_amt}")
            ...
            logging.info(f"Portal Old Vehicle - Chassis: '{chassis_val}', Reg No: '{reg_val}'")
            ...
            for text, field_name in [(chassis_val, "Chassis No"), (reg_val, "Reg No")]:
                ...
                if pan:
                    ...
                    logging.info(f"PAN card format detected in portal Old Vehicle {field_name}: {pan}")
                ...
                if dl:
                    ...
                    portal_pan_dl_source_field = field_name
                    logging.info(f"Driving Licence format detected in portal Old Vehicle {field_name}: {dl}")
            ...
            excel_records.append({
                ...
                "status": "APPROVED" if not row_issues else "HOLD",
        ...
        # Create Excel results sheet
        excel_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "kyc_process_results.xlsx")
        create_excel_results(excel_records, excel_path)
