...
try:
    ...
    load_dotenv(dotenv_path=os.path.join(get_bundled_dir(), ".env"))
except ImportError:
...
def select_antd_dropdown_option(
    ...
    if select_first:
        ...
        options_data[0][1].click(force=True)
...
OPENAI_CACHE = {}  # Cleared on each run to ensure visual extractions are generated
...
1. STEP 1: First, scan the ENTIRE document from top to bottom to understand its structure.
2. STEP 2: Identify the document type based on headers, logos, and keywords.
3. STEP 3: For INVOICES and DISCLAIMERS, locate these MANDATORY fields FIRST:
   - Invoice Number (usually labeled "Invoice No", "Invoice #", "Bill No")
   - Invoice Date (usually labeled "Invoice Date", "Date", "Bill Date")
   - Chassis Number (17-character alphanumeric, starts with MA1 or similar)
   - Customer Name (look near "To:", "Customer Name", "Buyer Name")
4. STEP 4: Read invoice_number and chassis_number CHARACTER BY CHARACTER:
   - Start from the leftmost character
   - Read each character exactly as it appears
   - Do NOT skip, rearrange, or guess characters
   - Common OCR mistakes to avoid:
     ...
     * Letter 'B' vs digit '8'
     * Letter 'S' vs digit '5'
5. Extract only information VISIBLE in the image - do NOT guess.
6. If a field is not clearly visible or missing, return null.
7. Preserve original spelling and formatting EXACTLY as shown.
8. Return ONLY valid JSON - no markdown, explanations, or code blocks.
9. If multiple values exist for a field, return them as an array.
10. Set confidence_score based on image quality:
    - 90-100: Crystal clear, all text readable
...
Wrong: "INV202600123" (removed formatting)

Example 2 - Chassis Number:
Image shows: "Chassis No: MA1TA2E3BM5K67890"
Correct: "chassis_number": "MA1TA2E3BM5K67890"
Wrong: "MA1TA223BM5K67890" (confused E with 2)

Example 3 - Welcome Bonus:
Image shows: "Note: Welcome Bonus Amount is Rs.5000.00/- (inclusive of GST)"
Correct: "welcome_bonus_amount": "5000"
Wrong: Do NOT extract grand total or taxable amount
...
def extract_details_via_openai(pdf_path, base64_images=None):
    """Converts PDF pages to base64 PNG images with preprocessing and calls the OpenAI Vision API."""
    print("\n" + "=" * 70)
    print("[VISUAL] extract_details_via_openai CALLED")
    print(f"[VISUAL] PDF: {pdf_path}")
    print("=" * 70 + "\n")
    ...
    def preprocess_image_for_vision(pil_image):
        ...
        try:
            ...
            pil_image = enhancer.enhance(1.3)  # Increase contrast by 30%

            # Enhance sharpness
            enhancer = ImageEnhance.Sharpness(pil_image)
            pil_image = enhancer.enhance(1.5)  # Increase sharpness by 50%

            # Slight denoising (reduce noise while preserving text)
            # Using median filter for handwritten text preservation
    ...
    if base64_images is None:
        ...
        try:
            ...
            for page_num in range(len(doc)):
                ...
                # Apply preprocessing to enhance quality
                ...
                logging.debug(
                    f"Preprocessed and encoded page {page_num + 1} of {pdf_path}"
                )
        except Exception as err:
...
Example 1 - Invoice Number:
Image shows: "Invoice No: INV-2026-00123"
Correct extraction: "invoice_number": "INV-2026-00123"
Wrong: "INV202600123" (removed formatting)

Example 2 - Chassis Number:
Image shows: "Chassis No: MA1TA2E3BM5K67890"
Correct extraction: "chassis_number": "MA1TA2E3BM5K67890"
Wrong: "MA1TA223BM5K67890" (confused E with 2)

Example 3 - Welcome Bonus:
Image shows: "Note: Welcome Bonus Amount is Rs.5000.00/- (inclusive of GST)"
Correct extraction: "welcome_bonus_amount": "5000"
Wrong: Do NOT extract grand total or taxable amount

IMPORTANT - BOUNDING BOXES FOR VISUAL CONFIRMATION:
For each field you extract, also provide the bounding box coordinates [x_min, y_min, x_max, y_max] in pixels.
This allows the UI to show cropped images of what you detected for visual verification.
...
""".replace("{filename_hint}", filename_hint)
    ...
    payload = {
        ...
        "top_p": 0.1,  # Low top_p for more focused, consistent outputs
    }

    # Retry logic with validation for critical fields
    max_retries = 3
    min_confidence_threshold = 60  # Minimum acceptable confidence score
    ...
    for retry_attempt in range(max_retries):
        try:
            ...
            response = requests.post(
                ...
                headers=headers,
                json=payload,
                timeout=90,  # Increased timeout from 60s
            )
            ...
            doc_type = str(extracted_data.get("document_type", "")).upper()
            confidence = extracted_data.get("confidence_score", 0)

            # Define critical fields based on document type
            critical_fields = []
            ...
            elif doc_type in ["PAN", "AADHAAR", "DL"]:
                critical_fields = ["document_number", "customer_name", "dob"]
            elif "COD" in doc_type:
                critical_fields = ["registration_number", "chassis_number"]

            # Check if critical fields are present and not null/empty
            missing_critical = []
            ...
            # If confidence is too low or critical fields are missing, retry
            if (
                confidence < min_confidence_threshold
                and retry_attempt < max_retries - 1
            ):
                logging.warning(
                    f"Low confidence score ({confidence}) for {os.path.basename(pdf_path)}. Retrying..."
                )
                continue

            if missing_critical and retry_attempt < max_retries - 1:
                logging.warning(
                    f"Missing critical fields {missing_critical} for {os.path.basename(pdf_path)}. Retrying..."
                )
                continue

            # Log warnings if still have issues on final attempt
            if missing_critical:
                logging.warning(
                    f"Warning: Missing critical fields after all retries: {missing_critical}"
                )
            if confidence < min_confidence_threshold:
                logging.warning(
                    f"Warning: Low confidence score after all retries: {confidence}"
                )

            logging.info(
                f"Successfully extracted {doc_type} data (confidence: {confidence}%) from {os.path.basename(pdf_path)}"
            )

            # Add visual confirmations (NEW!)
            logging.warning("[VISUAL] Starting visual confirmation extraction...")
            logging.warning(f"[VISUAL] PDF path: {pdf_path}")
            logging.warning(
                f"[VISUAL] Base64 images available: {len(base64_images) if base64_images else 0}"
            )

            try:
                visual_data = extract_visual_confirmations(
                    pdf_path, extracted_data, base64_images
                )
                logging.warning(
                    f"[VISUAL] Extraction returned {len(visual_data)} fields"
                )
                extracted_data["visual_extractions"] = visual_data
                logging.warning(
                    f"[VISUAL] ✓ Added {len(visual_data)} visual confirmations to extraction result"
                )
            except Exception as ve:
                logging.error(
                    f"[VISUAL] ✗ Failed to extract visual confirmations: {ve}"
                )
                import traceback
...
def extract_visual_confirmations(pdf_path, extracted_data, base64_images):
    ...
    and save them as base64 images for visual confirmation in UI.

    Uses EasyOCR fallback if GPT doesn't provide bounding boxes.
    ...
    logging.warning("[VISUAL] extract_visual_confirmations CALLED")

    visual_extractions = {}

    # Get bounding boxes from extraction result
    bounding_boxes = extracted_data.get("bounding_boxes", {})
    logging.warning(f"[VISUAL] GPT provided {len(bounding_boxes)} bounding boxes")

    # 1. Try EasyOCR first for mathematically perfect text bounding boxes
    logging.warning("[VISUAL] Running EasyOCR for perfect text bounding boxes...")
    bounding_boxes = extract_bboxes_with_easyocr(extracted_data, base64_images)

    # 2. Try OpenCV blue/purple ink detection for stamps and signatures
    if base64_images:
        try:
            import numpy as np
            import base64
            ...
            ink_boxes = find_visual_ink_bboxes(img_array)
            for k, v in ink_boxes.items():
                if k not in bounding_boxes:
                    bounding_boxes[k] = v
                    logging.warning(f"[VISUAL] OpenCV Ink detector found {k} at {v}")
        except Exception as e:
            logging.error(f"[VISUAL] OpenCV Ink detection failed: {e}")

    # 3. Fallback to OCR for Stamp (if not found by ink detector)
    if "seal_stamp_dealer_name" not in bounding_boxes and extracted_data.get("seal_stamp_dealer_name"):
        dealer_name = extracted_data.get("seal_stamp_dealer_name", "").strip().upper().replace(" ", "")
        if dealer_name and dealer_name not in ["NULL", "NONE", "NOT_FOUND"]:
            best_stamp_score = 0
            best_stamp_box = None
            try:
                import easyocr
                ...
                for bbox, text, conf in ocr_results:
                    ...
                    score = fuzz.ratio(text_clean, dealer_name)
                    if len(text_clean) > len(dealer_name):
                        score = max(score, fuzz.partial_ratio(dealer_name, text_clean))
                    if len(text_clean) > 3:
                        if dealer_name in text_clean: score = max(score, 90)
                        elif text_clean in dealer_name and len(text_clean) >= (len(dealer_name) * 0.6): score = max(score, 90)
                    
                    if score > 70:
                        y_coords = [pt[1] for pt in bbox]
                        avg_y = sum(y_coords) / len(y_coords)
                        # Boost score if it's lower on the page (stamps are at the bottom, headers are at the top)
                        adjusted_score = score + (avg_y / 100.0) 
                        if adjusted_score > best_stamp_score:
                            best_stamp_score = adjusted_score
                            x_coords = [pt[0] for pt in bbox]
                            best_stamp_box = [min(x_coords), min(y_coords), max(x_coords), max(y_coords)]
                
                if best_stamp_box:
                    bounding_boxes["seal_stamp_dealer_name"] = best_stamp_box
                    logging.warning(f"[VISUAL] Found stamp via OCR: {best_stamp_box}")
            except Exception as e:
                logging.error(f"[VISUAL] Stamp OCR fallback failed: {e}")

    # 4. Fallback to OCR for Signature (finding the label "Signature" and shifting up)
    if "customer_signature" not in bounding_boxes:
        try:
            # We already have ocr_results if we ran the stamp fallback, but just in case:
            best_sig_score = 0
            best_sig_box = None
            for bbox, text, conf in ocr_results:
                if conf < 0.3: continue
                t = text.lower()
                if "signature" in t or "sign" in t:
                    y_coords = [pt[1] for pt in bbox]
                    avg_y = sum(y_coords) / len(y_coords)
                    # Signatures are usually at the bottom
                    adjusted_score = 100 + (avg_y / 100.0)
                    if adjusted_score > best_sig_score:
                        best_sig_score = adjusted_score
                        x_coords = [pt[0] for pt in bbox]
                        # Expand box UPWARDS by 80 pixels to catch the handwritten ink above the text
                        best_sig_box = [min(x_coords), max(0, min(y_coords) - 80), max(x_coords) + 100, max(y_coords)]
            
            if best_sig_box:
                bounding_boxes["customer_signature"] = best_sig_box
                logging.warning(f"[VISUAL] Found signature label via OCR: {best_sig_box}")
        except Exception as e:
            logging.error(f"[VISUAL] Sig OCR fallback failed: {e}")

    if not bounding_boxes:
        logging.warning("[VISUAL] No bounding boxes available - returning empty")
        return visual_extractions
    ...
    try:
        ...
        logging.info(
            f"Extracting visual confirmations from image {img_width}x{img_height}"
        )
        ...
        for field_name, bbox in bounding_boxes.items():
            ...
            logging.info(
                f"✓ Visual confirmation extracted for {field_name} (bbox: {x1},{y1} to {x2},{y2})"
            )
...
def extract_bboxes_with_object_detection(base64_images):
    """
    Use GPT-4o-mini with object detection prompt to find bounding boxes.
    More reliable than asking for bboxes in the main extraction.
    """
    import base64
    import io
    import json
...
- bounding_box (ymin, xmin, ymax, xmax) - NORMALIZED FROM 0.0 to 1.0
- description

IMPORTANT:
- bounding_box MUST be normalized float values between 0.0 and 1.0. 
  (e.g., if an object is exactly in the center, ymin=0.45, xmin=0.45, ymax=0.55, xmax=0.55).
- (0.0, 0.0) is the top-left corner of the image. (1.0, 1.0) is the bottom-right.
- Bounding boxes should tightly enclose the object.
- If not found, return found=false and bounding_box=null.
...
JSON Schema:
{
  "objects": [
    {
      "object_type": "invoice_number",
      "found": true,
      "confidence": 0.95,
      "bounding_box": {"ymin": 0.1, "xmin": 0.2, "ymax": 0.15, "xmax": 0.3},
      "description": "INV-12345"
    }
  ]
}"""
        ...
        logging.warning("[VISUAL] Calling GPT-4o-mini for object detection...")
        response = requests.post(
            "https://api.openai.com/v1/chat/completions",
            headers=headers,
            json=payload,
        ...
        for obj in detection.get("objects", []):
            ...
            # Map object types to field names
            obj_type = obj.get("object_type", "").lower().replace(" ", "_")
            field_mapping = {
                "handwritten_signature": "customer_signature",
                "signature": "customer_signature",
                "company_stamp": "seal_stamp_dealer_name",
                "company_seal": "seal_stamp_dealer_name",
                "stamp": "seal_stamp_dealer_name",
                "seal": "seal_stamp_dealer_name",
            }

            field_name = field_mapping.get(obj_type, obj_type)

            # Convert normalized [ymin, xmin, ymax, xmax] to absolute [x1, y1, x2, y2]
            try:
                ymin = float(bbox_data.get("ymin", 0.0))
                xmin = float(bbox_data.get("xmin", 0.0))
                ymax = float(bbox_data.get("ymax", 0.0))
                xmax = float(bbox_data.get("xmax", 0.0))
                ...
                # Add a small 10px padding for better visual cropping
                x1 = max(0, x1 - 10)
                y1 = max(0, y1 - 10)
                x2 = min(img_width, x2 + 10)
                y2 = min(img_height, y2 + 10)

                bboxes[field_name] = [x1, y1, x2, y2]
                logging.warning(f"[VISUAL] Found {obj_type} at absolute bbox {bboxes[field_name]}")
            except Exception as e:
                logging.error(f"[VISUAL] Error converting normalized coordinates for {obj_type}: {e}")

        logging.warning(f"[VISUAL] Object detection found {len(bboxes)} bounding boxes")
        return bboxes

    except Exception as e:
        logging.error(f"[VISUAL] Object detection failed: {e}")
        return {}


def extract_bboxes_with_easyocr(extracted_data, base64_images):
    """
    Fallback: Use EasyOCR to find bounding boxes for extracted values.

    Returns: dict of {field_name: [x1, y1, x2, y2]}
    """
    import base64
    import io
    ...
    try:
        ...
        logging.info("Running EasyOCR to find bounding boxes...")
        reader = easyocr.Reader(["en"], gpu=False)
        ocr_results = reader.readtext(img_array)

        # Fields to find using exact text OCR
        fields_to_find = {
            "invoice_number": extracted_data.get("invoice_number", ""),
            "chassis_number": extracted_data.get("chassis_number", ""),
            "customer_name": extracted_data.get("customer_name", ""),
            "invoice_date": extracted_data.get("invoice_date", ""),
            "date": extracted_data.get("date", ""),  # Also check 'date' key
            "registration_number": extracted_data.get("registration_number", ""),
        }

        # Find matching OCR boxes for each field
        for field_name, target_value in fields_to_find.items():
            if not target_value or str(target_value).lower() in [
                "null",
                "none",
                "not_found",
                "",
            ...
            # Clean target value
            target_clean = str(target_value).strip().upper().replace(" ", "")
            ...
            for bbox, text, conf in ocr_results:
                ...
                score = fuzz.ratio(text_clean, target_clean)

                # Use partial_ratio to find the target inside a longer OCR line (even with 1-2 typos)
                if len(text_clean) > len(target_clean):
                    partial_score = fuzz.partial_ratio(target_clean, text_clean)
                    score = max(score, partial_score)
                
                # Also try direct substring match for perfection
                if len(text_clean) > 3:
                    if target_clean in text_clean:
                        score = max(score, 90)
                    elif text_clean in target_clean and len(text_clean) >= (len(target_clean) * 0.6):
                        score = max(score, 90)
            ...
            if best_bbox:
                bounding_boxes[field_name] = best_bbox
                logging.info(
                    f"  ✓ Found bbox for {field_name} (match: {best_match_score}%)"
                )

        # Special handling for stamps/signatures (look for visual elements)
        if "seal_stamp_dealer_name" not in bounding_boxes:
            # Try to find stamp-like regions (circular or rectangular high-density areas)
            stamp_bbox = find_stamp_region(img_array)
            if stamp_bbox:
                bounding_boxes["seal_stamp_dealer_name"] = stamp_bbox
                logging.info("  ✓ Found potential stamp region")

        logging.info(f"EasyOCR found {len(bounding_boxes)} bounding boxes")
        return bounding_boxes
...
def find_visual_ink_bboxes(img_array):
    """
    Finds blue/purple ink clusters (stamps and signatures) in the image.
    Returns a dict with 'customer_signature' and 'seal_stamp_dealer_name' bboxes.
    """
    try:
        import cv2
        import numpy as np

        # Convert to HSV to find blue/purple ink
        ...
        for i in range(1, num_labels):
            ...
            if 150 < area < 50000:  # Ignore tiny noise and massive blocks
        ...
        # Merge overlapping/nearby clusters (within 100 pixels)
        merged = []
        ...
        # Sort merged clusters by area (largest first)
        merged = sorted(merged, key=lambda item: item[4], reverse=True)
        
        res = {}
        # The largest blue cluster is usually the stamp
        if len(merged) > 0:
            mx, my, mw, mh, ma = merged[0]
            res["seal_stamp_dealer_name"] = [mx, my, mx+mw, my+mh]
        # The second largest is usually the signature
        if len(merged) > 1:
            mx, my, mw, mh, ma = merged[1]
            res["customer_signature"] = [mx, my, mx+mw, my+mh]

        return res
    except Exception as e:
        import logging
        logging.error(f"Error finding ink regions: {e}")
        return {}
...
def extract_text_hybrid(pdf_path):
    ...
    # Check if we have a cached result for this exact file (using file path + modification time)
    cache_key = pdf_path
    try:
        file_mtime = os.path.getmtime(pdf_path)
        file_size = os.path.getsize(pdf_path)
        cache_key = f"{pdf_path}_{file_mtime}_{file_size}"
    except Exception:
        cache_key = pdf_path

    # Initialize text extraction cache if it doesn't exist
    if not hasattr(extract_text_hybrid, "_text_cache"):
        extract_text_hybrid._text_cache = {}

    # Return cached result if available
    if cache_key in extract_text_hybrid._text_cache:
        logging.info("--> Using cached text extraction result.")
        return extract_text_hybrid._text_cache[cache_key]
    ...
    if openai_res:
        ...
        if "visual_extractions" in openai_res:
            print(
                f"[VISUAL DEBUG] Number of visual fields: {len(openai_res.get('visual_extractions', {}))}"
            )
    ...
    if openai_res:
        ...
        if is_pan:
            ...
            if not vehicle_make or vehicle_make == "NOT_FOUND":
                vehicle_make = openai_res.get("dealership_name") or openai_res.get("company_name")
            ...
            dealer_name = openai_res.get("dealership_name") or openai_res.get("company_name")
        ...
        if "visual_extractions" in openai_res:
            res_dict["extracted_data"]["visual_extractions"] = openai_res["visual_extractions"]
...
def count_signatures_in_mask(mask_img):
    import cv2

    num_labels, _, stats, _ = cv2.connectedComponentsWithStats(mask_img)
    clusters = []
    for i in range(1, num_labels):
        area = stats[i, cv2.CC_STAT_AREA]
        if area > 50:
...
def verify_ledger_stamp_and_signature(pdf_path, company_name):
    ...
    try:
        ...
        sig_count = count_signatures_in_mask(mask)
        print(
            f"\033[92m[Signature Detector] Ledger Document: Found {sig_count} signature(s).\033[0m"
        )
...
def verify_document_with_openai_and_portal(
    doc_path, claim_details, old_vehicle_details
):
    import base64
    import json
    import logging
    ...
    def get_first_page_b64(pdf_path):
        try:
            ...
            # Save the image locally per user request
            images_dir = os.path.join(os.path.dirname(pdf_path), "images")
            if not os.path.exists(images_dir):
                os.makedirs(images_dir)

            base_name = os.path.splitext(os.path.basename(pdf_path))[0]
            save_path = os.path.join(images_dir, f"{base_name}_page0.png")
            pix.save(save_path)
            logging.info(f"Saved converted PDF image to: {save_path}")
    ...
    img_b64 = get_first_page_b64(doc_path)
    if not img_b64:
        return {"document_status": "HOLD", "hold_reason": "Failed to render PDF image"}

    api_key = os.getenv("OPENAI_API_KEY", "")
    headers = {"Content-Type": "application/json", "Authorization": f"Bearer {api_key}"}

    # Extract portal details safely
    portal_claim_data = json.dumps(claim_details or {}, indent=2)
    portal_old_vehicle_data = json.dumps(old_vehicle_details or {}, indent=2)

    prompt = f"""You are an expert document analysis and validation AI.

Analyze the entire document carefully. Perform the following signature and stamp validation strictly based on visual evidence. Do not guess.

### Customer Signature Validation - CRITICAL RULES
* Locate the field labeled "Customer Signature" or any similar wording.
* IMPORTANT: Only mark "present: true" if you see an ACTUAL HANDWRITTEN SIGNATURE OR SCRIBBLE in the customer signature area.
* If the field says "NO" next to "Customer Signature", then mark "present: false".
* If the signature area is completely blank or empty, mark "present: false".
* DO NOT confuse printed text, stamps, or other signatures with the customer signature.
* DO NOT mark it as present just because you see the label "Customer Signature" - you must see the actual handwritten signature itself.
* The customer signature is typically found in a dedicated area/box separate from dealer/authorized signatures.
* If you are uncertain or cannot clearly see a handwritten customer signature, mark "present: false" and set confidence low.
...
### Company Stamp Validation
* Detect whether an official company stamp or seal is present anywhere on the document.
* The stamp may be circular, rectangular, oval, blue, black, violet, or another common ink color.
* The stamp may partially overlap signatures or printed text.
* Do not assume a stamp exists simply because a company name is printed.
* Only report a stamp as present if it is visually detectable (ink/impression visible).
...
If ANY of the above are missing, set:
Document Status = HOLD
Otherwise:
Document Status = PASS

Return the validation results in JSON using this strict structure exactly as shown:
{{
  "customer_signature": {{
    "present": true,
    "confidence": 0.98
...
If the document is placed on HOLD, clearly specify which required item(s) are missing in the "hold_reason" field.
Never guess. Base every decision only on what is visually present in the document.
Be extremely careful not to mark customer signature as present if it's not actually there.
"""
...
### Customer Name Extraction - CRITICAL INSTRUCTIONS
1. Extract the printed CUSTOMER name from Image 1 (Invoice):
   - Look for the customer name in the "Bill To" section, "Customer Name" field, or "Buyer" section
   - DO NOT extract the dealer/seller/company name from the invoice header/letterhead
   - DO NOT extract dealership names like "SHREE R. C. ENTERPRISES", "XYZ MOTORS", etc.
   - Extract only the individual customer's name (e.g., "SACHIN KUMAR SINGH")
   - If not found, return null.

2. Extract the printed CUSTOMER name from Image 2 (Declaration/Disclaimer):
   - Look for the printed name near or below the "Customer Signature" label/field
   - This is typically printed in capital letters below the handwritten signature
   - DO NOT extract the dealership name that appears in the declaration body text (e.g., "SHREE RC ENTERPRISES")
   - DO NOT extract company names from phrases like "from the Dealership name ___"
   - Extract only the customer's printed name (e.g., "SACHIN KUMAR SINGH")
   - If not found, return null.

3. Detect and extract the handwritten customer name/signature from Image 1 (Invoice):
   - Look for handwritten text in the customer signature area
   - Usually appears as cursive/handwritten scribbles
   - If not present, return null.

4. Detect and extract the handwritten customer name/signature from Image 2 (Declaration/Disclaimer):
   - Look for handwritten text in the customer signature area (usually on the right side)
   - The handwritten signature typically appears ABOVE the printed customer name
   - Usually appears as cursive/handwritten scribbles
   - If not present, return null.

### Stamp Presence Check - CRITICAL INSTRUCTIONS
1. Check whether a dealer seal/company seal/dealer stamp is present on Image 1 (Invoice):
   - Stamps can be: circular, rectangular, oval, or irregular shapes
   - Stamps can be in: blue ink, purple ink, black ink, red ink, or faded/light colors
   - Stamps can appear: overlapping signatures, in corners, at bottom, partially visible, faint
   - The stamp may contain text like company name, "Authorized Dealer", or be decorative
   - DO NOT require the stamp text to be clearly readable - even faint/blurry stamps count
   - Look carefully at the ENTIRE document - stamps can appear anywhere
   - If you see ANY ink impression that looks like a stamp/seal, return true
   - Return true or false.

2. Check whether a dealer seal/company seal/dealer stamp is present on Image 2 (Declaration/Disclaimer):
   - Same rules as above - stamps can be circular, rectangular, oval shapes
   - Stamps are typically found near the dealer signature area (left/bottom area)
   - The stamp may overlap with "For [Company Name]" signature section
   - Stamps can be faint, partial, or low quality - still count as present
   - Look at the LEFT side of the document where dealer signature typically appears
   - DO NOT confuse QR codes or printed logos with stamps - stamps are ink impressions
   - If you see ANY circular/rectangular ink impression near dealer area, return true
   - Return true or false.
...
"""
    ...
    try:
        ...
        # Validation: Check if OpenAI mistakenly extracted dealership names instead of customer names
        # Common dealership keywords that should NOT appear in customer names
        dealership_keywords = [
            "ENTERPRISES",
            "MOTORS",
            "AUTO",
        ...
        # Check printed_name_disclaimer
        if p_dec:
            p_dec_upper = str(p_dec).upper()
            if any(keyword in p_dec_upper for keyword in dealership_keywords):
                logging.warning(
                    f"OpenAI extracted dealership name as customer name in disclaimer: '{p_dec}'. Setting to null."
                )
                p_dec = None
                gpt_res["printed_name_disclaimer"] = None
                if "remarks" not in gpt_res or not isinstance(gpt_res["remarks"], list):
                    gpt_res["remarks"] = []
                gpt_res["remarks"].append(
                    "WARNING: AI extracted dealership name instead of customer name in disclaimer. Corrected to null."
                )

        # Check printed_name_invoice (less common but possible)
        if p_inv:
            p_inv_upper = str(p_inv).upper()
            if any(keyword in p_inv_upper for keyword in dealership_keywords):
                logging.warning(
                    f"OpenAI extracted dealership name as customer name in invoice: '{p_inv}'. Setting to null."
                )
                p_inv = None
                gpt_res["printed_name_invoice"] = None
                if "remarks" not in gpt_res or not isinstance(gpt_res["remarks"], list):
                    gpt_res["remarks"] = []
                gpt_res["remarks"].append(
                    "WARNING: AI extracted dealership name instead of customer name in invoice. Corrected to null."
                )
        ...
        # OCR-based fallback: If p_dec is still null (OpenAI failed to extract customer name from disclaimer),
        # try using OCR to find the printed name below "Customer Signature"
        if not p_dec:
            try:
                import cv2
                import numpy as np
                ...
                logging.info(
                    "OpenAI failed to extract customer name from disclaimer. Trying OCR fallback..."
                )

                doc_disc = fitz.open(declaration_path)
                page_disc = doc_disc.load_page(0)
                pix_disc = page_disc.get_pixmap(dpi=200)
                img_disc = cv2.imdecode(
                    np.frombuffer(pix_disc.tobytes("png"), np.uint8), cv2.IMREAD_COLOR
                )
                reader = get_ocr_reader()
                ocr_results = reader.readtext(img_disc, paragraph=False)

                # Look for "Customer Signature" or "SACHIN KUMAR SINGH" pattern
                for i, (box, txt, conf) in enumerate(ocr_results):
                    txt_upper = txt.strip().upper()
                    if "CUSTOMER" in txt_upper and "SIGNATURE" in txt_upper:
                        # Look at the next few OCR boxes for the printed customer name
                        for j in range(i + 1, min(i + 5, len(ocr_results))):
                            next_box, next_txt, next_conf = ocr_results[j]
                            next_txt_clean = next_txt.strip()
                            # Check if it looks like a name (not dealership keywords)
                            if len(next_txt_clean) > 5 and next_conf > 0.5:
                                next_upper = next_txt_clean.upper()
                                is_dealership = any(
                                    kw in next_upper
                                    for kw in [
                                        "ENTERPRISES",
                                ...
                                if not is_dealership:
                                    # Check if it's mostly alphabetic (names should be)
                                    alpha_count = sum(
                                        c.isalpha() or c.isspace()
                                        for c in next_txt_clean
                                    )
                                    if alpha_count >= len(next_txt_clean) * 0.7:
                                        p_dec = next_txt_clean
                                        gpt_res["printed_name_disclaimer"] = p_dec
                                        logging.info(
                                            f"OCR fallback extracted customer name from disclaimer: '{p_dec}'"
                                        )
                                        remarks.append(
                                            f"Customer name extracted via OCR fallback: {p_dec}"
                                        )
                                        break
                        if p_dec:
                            break
                ...
                logging.warning(
                    f"OCR fallback for disclaimer customer name failed: {e}"
                )
        ...
        # CV-based stamp detection fallback: If OpenAI says stamps are missing, double-check with computer vision
        if not stamp_inv or not stamp_dec:
            try:
                import cv2
                import numpy as np

                logging.info(
                    "OpenAI reported missing stamps. Running CV-based stamp detection fallback..."
                )

                def detect_stamp_cv(pdf_path, doc_name):
                    """Use color-based detection to find circular/rectangular stamp shapes"""
                    doc = fitz.open(pdf_path)
                    page = doc.load_page(0)
                    ...
                    # Detect blue/purple stamps (most common)
                    lower_blue = np.array([90, 50, 50])
                    upper_blue = np.array([130, 255, 255])
                    mask_blue = cv2.inRange(hsv, lower_blue, upper_blue)

                    # Detect black/dark stamps
                    ...
                    # Find connected components (potential stamps)
                    num_labels, labels, stats, centroids = (
                        cv2.connectedComponentsWithStats(mask_combined)
                    )

                    # Look for stamp-sized components (circular or rectangular shapes)
                    for i in range(1, num_labels):
                        area = stats[i, cv2.CC_STAT_AREA]
                        if 500 < area < 50000:  # Typical stamp size range
                            x = stats[i, cv2.CC_STAT_LEFT]
                            y = stats[i, cv2.CC_STAT_TOP]
                            w_c = stats[i, cv2.CC_STAT_WIDTH]
                            h_c = stats[i, cv2.CC_STAT_HEIGHT]

                            # Check aspect ratio (stamps are typically circular or squarish)
                            aspect_ratio = w_c / h_c if h_c > 0 else 0
                            if 0.5 <= aspect_ratio <= 2.0:
                                logging.info(
                                    f"CV detected potential stamp in {doc_name}: area={area}, pos=({x},{y})"
                                )
                                return True
                ...
                if not stamp_inv:
                    cv_stamp_inv = detect_stamp_cv(invoice_path, "Invoice")
                    if cv_stamp_inv:
                        logging.warning(
                            f"OpenAI missed stamp in Invoice. CV detection overriding to True."
                        )
                        stamp_inv = True
                        gpt_res["stamp_present_invoice"] = True
                        remarks.append(
                            "Stamp detected in invoice via CV fallback (OpenAI missed it)"
                        )

                if not stamp_dec:
                    cv_stamp_dec = detect_stamp_cv(declaration_path, "Disclaimer")
                    if cv_stamp_dec:
                        logging.warning(
                            f"OpenAI missed stamp in Disclaimer. CV detection overriding to True."
                        )
                        stamp_dec = True
                        gpt_res["stamp_present_disclaimer"] = True
                        remarks.append(
                            "Stamp detected in disclaimer via CV fallback (OpenAI missed it)"
                        )

            except Exception as e:
                logging.warning(f"CV-based stamp detection fallback failed: {e}")
        ...
        if not stamp_present:
            ...
            if not stamp_inv and not stamp_dec:
                remarks.append(
                    "Stamp is missing on both invoice and disclaimer documents."
                )
            elif not stamp_inv:
                remarks.append("Stamp is missing on invoice document.")
            elif not stamp_dec:
                remarks.append("Stamp is missing on disclaimer document.")
            else:
...
def verify_invoice_stamp_and_signatures(
    pdf_path, company_name, customer_name, doc_type="Invoice/Disclaimer"
):
    logging.info(
    ...
    try:
        ...
        sig_count = count_signatures_in_mask(sig_region_combined)
        print(
            f"\033[92m[Signature Detector] {doc_type} Document: Found {sig_count} signature(s).\033[0m"
        )
        ...
        stamp_count = len(merged)
        print(
            f"\033[92m[Stamp Detector] {doc_type} Document: Found {stamp_count} dealership seal/stamp(s).\033[0m"
        )
        ...
        # Additional OCR-based check: Look for "Customer Signature: NO" or similar patterns
        customer_sig_no_detected = False
        for i, (box, txt, conf) in enumerate(ocr_results):
            txt_clean = txt.strip().upper()
            if "CUSTOMER" in txt_clean and "SIGNATURE" in txt_clean:
                # Look at the next few OCR boxes for "NO", "N/A", "NIL"
                for j in range(i, min(i + 5, len(ocr_results))):
                    next_txt = ocr_results[j][1].strip().upper()
                    if (
                        next_txt in ["NO", "N/A", "NIL", "NA", "NONE"]
                        or ":NO" in next_txt
                        or ": NO" in next_txt
                    ):
                        customer_sig_no_detected = True
                        logging.info(
                            f"OCR detected 'Customer Signature: {next_txt}' - Overriding signature detection"
                        )
                        break
                if customer_sig_no_detected:
                    break

        # Override sig_ok if OCR detected "NO"
        if customer_sig_no_detected:
            sig_ok = False
            sig_msg = (
                "NOT DETECTED (Customer Signature field explicitly marked as NO/N/A)"
            )
...
def validate_disclaimer_doc(
    ...
    sig_ok, sig_msg, stamp_ok, stamp_msg = verify_invoice_stamp_and_signatures(
        pdf_path, expected_company, customer_name, doc_type="Disclaimer"
    )
...
def validate_invoice_doc(
    ...
    # 1. Invoice Type check (with improved fuzzy matching to handle OCR variations)
    has_tax_invoice = "TAX INVOICE" in text_upper
    has_gst_invoice = "GST INVOICE" in text_upper
    has_proforma = "PROFORMA" in text_upper

    # Also check for common OCR variations and spacing issues
    if not has_tax_invoice:
        has_tax_invoice = (
            "TAXINVOICE" in text_upper
            or "TAX-INVOICE" in text_upper
            or "TAX  INVOICE" in text_upper
            or "TAX   INVOICE" in text_upper
            or (
                "TAX" in text_upper
                and "INVOICE" in text_upper
                and abs(text_upper.find("TAX") - text_upper.find("INVOICE")) < 20
            )
        )

    if not has_gst_invoice:
        has_gst_invoice = (
            "GSTINVOICE" in text_upper
            or "GST-INVOICE" in text_upper
            or "GST  INVOICE" in text_upper
            or "GST   INVOICE" in text_upper
            or (
                "GST" in text_upper
                and "INVOICE" in text_upper
                and abs(text_upper.find("GST") - text_upper.find("INVOICE")) < 20
            )
        )
    ...
    # Debug logging to help diagnose intermittent issues
    logging.info(
        f"Invoice type check for {filename}: has_tax_invoice={has_tax_invoice}, has_gst_invoice={has_gst_invoice}"
    )
    if not type_check_ok:
        # Extract a snippet around "TAX" and "INVOICE" keywords for debugging
        tax_idx = text_upper.find("TAX")
        inv_idx = text_upper.find("INVOICE")
        debug_snippet = ""
        if tax_idx >= 0:
            debug_snippet += f"TAX context: '{text_upper[max(0, tax_idx - 10) : min(len(text_upper), tax_idx + 20)]}' | "
        if inv_idx >= 0:
            debug_snippet += f"INVOICE context: '{text_upper[max(0, inv_idx - 10) : min(len(text_upper), inv_idx + 30)]}'"
        logging.warning(f"Invoice type check failed. Debug: {debug_snippet}")
    ...
    # 6. Customer Signature, Dealer Seal & Portal Verification (OpenAI Vision)
    import json

    import cv2
    import numpy as np

    openai_res = verify_document_with_openai_and_portal(
        pdf_path, claim_details, old_vehicle_details
    )
    print("\n  --- OpenAI Vision Document Analysis ---")
    print(json.dumps(openai_res, indent=2))

    cust_sig = openai_res.get("customer_signature", {}).get("present")
    auth_sig = openai_res.get("authorized_signature", {}).get("present")
    comp_stamp = openai_res.get("company_stamp", {}).get("present")

    # OCR-based validation to detect "Customer Signature: NO" patterns
    try:
        doc_check = fitz.open(pdf_path)
        page_check = doc_check.load_page(0)
        pix_check = page_check.get_pixmap(dpi=200)
        img_check = cv2.imdecode(
            np.frombuffer(pix_check.tobytes("png"), np.uint8), cv2.IMREAD_COLOR
        )
        reader = get_ocr_reader()
        ocr_results_check = reader.readtext(img_check, paragraph=False)

        # Check if OCR finds "Customer Signature" followed by "NO", "N/A", "NIL", etc.
        customer_sig_no_detected = False
        for i, (box, txt, conf) in enumerate(ocr_results_check):
            txt_clean = txt.strip().upper()
            if "CUSTOMER" in txt_clean and "SIGNATURE" in txt_clean:
                # Look at the next few OCR boxes for "NO", "N/A", "NIL"
                for j in range(i, min(i + 5, len(ocr_results_check))):
                    next_txt = ocr_results_check[j][1].strip().upper()
                    if (
                        next_txt in ["NO", "N/A", "NIL", "NA", "NONE"]
                        or ":NO" in next_txt
                        or ": NO" in next_txt
                        ...
                        customer_sig_no_detected = True
                        print(
                            f"\n  ⚠️  OCR OVERRIDE: Detected 'Customer Signature: {next_txt}' - Marking as NOT FOUND"
                        )
                        break
                if customer_sig_no_detected:
                    break

        # Override OpenAI result if OCR detected "NO"
        if customer_sig_no_detected and cust_sig:
            print(
                f"  ⚠️  OpenAI incorrectly detected customer signature. OCR override applied."
            )
            cust_sig = False
            openai_res["customer_signature"]["present"] = False
            openai_res["customer_signature"]["ocr_override"] = True
            if openai_res.get("document_status") == "PASS":
                openai_res["document_status"] = "HOLD"
                openai_res["hold_reason"] = (
                    "Customer signature is missing (marked as NO/N/A)"
                )
    except Exception as e:
        logging.warning(f"OCR validation check failed: {e}")

    c_sig_str = (
        f"{GREEN_TEXT}Found{RESET_TEXT}"
        if cust_sig
        else f"{RED_TEXT}Not Found{RESET_TEXT}"
    )
    stamp_str = (
        f"{GREEN_TEXT}Found{RESET_TEXT}"
        if comp_stamp
    ...
    print(f"  - Invoice Customer Signature : {c_sig_str}")
    print(f"  - Invoice Stamp            : {stamp_str}")
    print(f"  - Signature on Stamp       : {a_sig_str}")

    if openai_res.get("document_status") == "HOLD":
        hold_r = openai_res.get("hold_reason") or "Missing signatures or stamps."
        issues.append(f"Invoice [{filename}]: OpenAI Validation Failed - {hold_r}")
        print(f"  - OpenAI Check         : {RED_TEXT}FAIL ({hold_r}){RESET_TEXT}")
    else:
        print(f"  - OpenAI Check         : {GREEN_TEXT}PASS{RESET_TEXT}")
...
def evaluate_relationship_documents(
    claim_name, old_owner_name, portal_relation, pdf_files, issues
):
    """
    Evaluates relationship based on names and portal_relation.
    Enforces supporting document presence (Aadhar/PAN for Family, GST/Affidavit for Proprietor/No Relation).
    """
    GREEN_TEXT = "\033[92m"
    RED_TEXT = "\033[91m"
    RESET_TEXT = "\033[0m"
    ...
    # 1. Self logic
    if claim_name and old_owner_name:
        if fuzz.token_sort_ratio(claim_name.lower(), old_owner_name.lower()) >= 80:
            print(
                f"  - Relationship Check   : {GREEN_TEXT}PASS (Matched as 'Self'){RESET_TEXT}"
            )
            return "Self"

    if not portal_relation:
        portal_relation = "Unknown"

    relation_upper = portal_relation.upper().strip()

    # Extract file types from pdf_files list (filenames)
    doc_types_found = []
    for f in pdf_files:
        fn = os.path.basename(f).upper()
        if "AADHAR" in fn or "AADHAAR" in fn:
            doc_types_found.append("AADHAR")
        if "PAN" in fn:
            doc_types_found.append("PAN")
        if "GST" in fn:
            doc_types_found.append("GST")
        if "AFFIDAVIT" in fn or "AFF" in fn:
            doc_types_found.append("AFFIDAVIT")

    # 2. Family relation (requires Aadhar or PAN)
    family_keywords = [
        "FATHER",
        "MOTHER",
        "SPOUSE",
    ...
    if any(k in relation_upper for k in family_keywords):
        if "AADHAR" in doc_types_found or "PAN" in doc_types_found:
            print(
                f"  - Relationship Check   : {GREEN_TEXT}PASS (Family '{portal_relation}' with supporting Aadhar/PAN found){RESET_TEXT}"
            )
        else:
            print(
                f"  - Relationship Check   : {RED_TEXT}FAIL (Family '{portal_relation}' but no Aadhar or PAN found){RESET_TEXT}"
            )
            issues.append(
                f"Relationship Document Missing: Claimed Family ({portal_relation}) but neither Aadhar nor PAN card is uploaded."
            )
        return portal_relation

    # 3. Proprietor / No Relation (requires GST or Affidavit)
    if (
        "PROPRIETOR" in relation_upper
        or "NO RELATION" in relation_upper
        or "DIRECTOR" in relation_upper
    ):
        if "GST" in doc_types_found or "AFFIDAVIT" in doc_types_found:
            print(
                f"  - Relationship Check   : {GREEN_TEXT}PASS ('{portal_relation}' with supporting GST/Affidavit found){RESET_TEXT}"
            )
        else:
            print(
                f"  - Relationship Check   : {RED_TEXT}FAIL ('{portal_relation}' but no GST or Affidavit found){RESET_TEXT}"
            )
            issues.append(
                f"Relationship Document Missing: Claimed '{portal_relation}' but neither GST Document nor Affidavit is uploaded."
            )
        return portal_relation

    print(
        f"  - Relationship Check   : {RED_TEXT}WARNING (Unrecognized relation type '{portal_relation}'){RESET_TEXT}"
    )
    return portal_relation
...
def common_verify_documents(
    ...
    relationship = evaluate_relationship_documents(
        customer_name, old_owner_name, relationship, pdf_files, issues
    )
...
def main(use_existing_login=None, target_claim_choice=None, row_limit=None, date_range_choice="Current Month", force_scheme=None):
    ...
    global OPENAI_CACHE
    OPENAI_CACHE = {}
    print("[VISUAL] Cleared OpenAI cache - will generate fresh visual extractions")
    ...
    try:
        ...
        try:
            ...
            if os.path.exists(local_state_path):
                ...
                if "Default" not in info_cache and os.path.exists(
                ...
                for folder_name, info in info_cache.items():
                    ...
                    profiles.append(
                        {"folder": folder_name, "name": name, "user_name": user_name}
                    )
        except Exception as e:
        ...
        if not profiles:
            profiles = [
                {"folder": "Default", "name": "Default Profile", "user_name": ""}
            ]
        ...
        while profile_dir is None:
            profile_choice = get_ui_input(
                f"Select a profile (1-{len(profiles)}): ", "choice", options
            ).strip()
            try:
                ...
                if 0 <= profile_idx < len(profiles):
                    ...
                    logging.info(
                        f"Selected Edge profile: {selected_name} (Folder: {profile_dir})"
                    )
                else:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logging.warning(
                    f"Failed to launch Edge (Attempt {attempt + 1}/{max_retries}): Profile is locked or Edge is still running."
                )
                if attempt < max_retries - 1:
                    logging.info(
                        "Force killing background msedge.exe processes and retrying..."
                    )
                    subprocess.run(
                        ["taskkill", "/F", "/IM", "msedge.exe"], capture_output=True
                    )
        ...
        logging.critical(
            f"CRITICAL ERROR: Failed to launch Microsoft Edge after multiple attempts."
        )
